...
| Ultimate | PARTIAL: one launcher island with alternative left/right landing islands, split acid/muncher rooms, ice pyramids, branching finish; left route and authored right hazard-room path pass with recoveries; exact original route coverage open | Contact-triggered hinged launcher arms and aimed impulses, curling munchers, deforming acid with provisional trajectories, ice, three sequential disappearing bridge tiles; original launcher law and exact bridge pattern open | 25 units added; six-course ending and 20,000 + 1,000/time − 1,000/fall award implemented; complete timed campaign acceptance open |
...
## Newly confirmed clock discrepancy for the next calibration pass

The original recording at 86.00s explicitly displays `EXTRA TIME FOR INTERMEDIATE RACE: + 40`; the current `COURSE_TIME.intermediate` and single-course definition still use 45. This is a confirmed mismatch, not a request to extend the clock. Correcting it also requires remeasuring the campaign and improving normal-control traversal where needed; the current timed-Silly regression relies on the extra five units. The full parity goal remains open.

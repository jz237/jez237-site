...
try {
 ...
 assert.equal((await read()).mode, 'xray', 'studio opens in X-ray'); for (const mode of ['skin', 'realistic', 'skinless', 'coded', 'clay', 'xray']) { await page.locator(`[data-mode="${mode}"]`).click(); assert.equal((await read()).mode, mode); if (mode === 'skinless') { const v = await page.evaluate(() => window.anatomyStudio.getParts().filter(p => p.visible).map(p => p.file)); assert.ok(v.length > 1000 && !v.includes('regions'), 'No skin hides every surface region and nothing else'); } }

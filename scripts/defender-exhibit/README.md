...
### Three-game positional audio

All three existing game sound engines unlock together and follow global SOUND/volume. Defender, Robotron and Joust retain their original sound-program samples and gameplay events. Selection no longer mutes the other games: the selected cabinet has full positional gain while both neighbors retain a quieter distance-attenuated mix, capped below the selected speaker even if a background cabinet is nearer the camera. Panning and rear filtering follow actual speaker transforms during cabinet swaps; gains ease between levels. Power and display pause still affect only the selected game. Spatial gains start silent until the first camera mix, avoiding an initial full-volume burst. No additional simulations or synthetic background loops are used.

...
| Game | Original bitmap frames and palette values; ten humans, background abductions/mutation, falling catches, pods/swarmers, mines, finite smart bombs, ship loss/respawn, extra ships, survivor bonuses, terrain loss/restoration and automatic game-over restart | Native recreation with an autonomous pilot; no original game CPU emulation. The pilot reacts to independent enemy and human state; motion, exact CPU timing, collision masks, terrain and color cycling remain approximations. Original character bitmaps replace the substitute font; radar uses live human positions and the actual viewport offset. First four wave counts follow the source, later waves reuse wave-four counts, with source-table inter-wave horizontal speed ratios for landers, mutants and swarmers rather than the full operator-adjustable difficulty table. Multiple-human carrying and delivery are implemented; human friendly fire remains unimplemented. |
| Sound | Original VSNDRM1 sound program executed through 6800/PIA/DAC emulation offline; eighteen rendered PCM effects replace the synth; original command numbers, catch repetitions and priorities | Rendered sample playback rather than live board emulation; repeated thrust loop, 30 Hz DC removal and short edge fades; not a full analog amplifier/speaker model |
...
- Production build passes TypeScript and Vite. 33 source/model/sound/sprite tests pass. The autonomous-display tests explicitly forbid ground pickups; a separate legacy playable-game test is not the displayed demo.
...
## Autonomous gameplay revision
- Original bitmap frames decoded from `defb6.src` into `DefenderSprites.ts`, including both ship directions, three lander/baiter frames, four bomber frames, humanoids, mines and HUD reserves. Static palette entries follow CRTAB; cycling entries are approximated. Source: https://github.com/mwenge/defender/blob/master/src/defb6.src . Original bitmap material remains Williams material.
- First four wave compositions follow WVTAB (15/20/20/20 landers, 0/3/4/5 bombers, 0/1/3/4 pods). Source: https://github.com/mwenge/defender/blob/master/src/blk71.src . Wave completion awards 100 × min(wave, 5) per surviving human, then displays a bonus interval. Extra ships/bombs use a 10,000-point setting. Human population restores every fifth wave.
- Added original-table bomb, player-death, free-ship and terrain-destruction sound sequences; the same verified sound-ROM renderer produces all 18 files.
- Review 1: corrected fixed HUD stock, missing deaths/bonuses and invented enemy sprites. Review 2: simulation exposed artificial lander repositioning and absent background abductions; landers now travel to humans and background carriers can mutate. Review 3: browser screenshots exposed empty decoded FDB sprite rows; fixed the decoder and added native-dimension/nonempty-data regression checks. Corrected sprites inspected on desktop and mobile.
- Ten-minute deterministic simulation exercises progression through multiple waves, rescues, finite bomb use, deaths and autonomous new-game cycles. Scenario tests cover planet loss/restoration, visible-area bombs, free ships and natural background abductions. The full original game's behavior is not claimed bit-for-bit.


### Flight and collision follow-up
- Added horizontal acceleration and braking: reversing facing no longer reverses velocity instantly. The virtual thrust input follows the pilot's acceleration command.
- Laser collision sweeps the full travelled segment and chooses the nearest target, including across the world seam. Hit rectangles now match native sprite extents more closely; exact per-pixel ROM collision masks remain unimplemented.
- Removed the foreground lander's artificial immunity before interception. Destroying it before pickup leaves the human on the ground.
- Scanner humanoids follow their actual carrier, falling, carried or ground position. Bonus transitions clear lasers/mines/projectiles and wait for outstanding rescues.
- New regression coverage includes reversal momentum, seam-crossing laser collisions, near misses, early lander destruction, scanner positions and transition cleanup. Original sound assets are unchanged.


### Reactive-pilot revision
- Removed the directed rescue mission and all pilot-driven lander positioning. `DefenderPilot.ts` reads a snapshot and outputs only desired ship movement, facing and fire. The game engine owns abductions, falls, catches, delivery, damage, bombs and spawning.
- The pilot selects reachable falling humans, can collect another while carrying, approaches low carriers before shooting, and chooses evasive maneuvers from nearby hazards. No hidden rescue script or forced passenger generation remains.
- Each humanoid occupies exactly one of grounded, lander-carried, falling, ship-carried or dead. Several humans can be caught and delivered; each awards its own catch/delivery points. Gentle unaided landings award 250; fast ground impacts kill. Ship loss releases passengers into ordinary falls.
- Landers arrive in squads of five, with remaining reserves blocking wave completion. Squad replenishment follows the source's no-active-landers / timer behavior; source wave counts and bounded inter-wave speed changes are retained. Baiters accelerate as the wave lasts; pursuing enemies turn with momentum; pods burst into red swarmers, and bomber mines remain stationary. The native movement/timing scale and difficulty caps are still approximations of the ROM routines.

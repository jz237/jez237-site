...
| Game | Original bitmap frames and palette values; ten humans, background abductions/mutation, falling catches, pods/swarmers, mines, finite smart bombs, ship loss/respawn, extra ships, survivor bonuses, terrain loss/restoration and automatic game-over restart | Native recreation with an autonomous pilot; no original game CPU emulation. Foreground rescues remain directed; motion, spawn timing, collision masks, font, radar, terrain and color cycling are approximations. First four wave counts follow the source, later waves reuse wave-four counts, with source-table inter-wave horizontal speed ratios for landers, mutants and swarmers rather than the full operator-adjustable difficulty table. Human friendly fire and multi-human carrying are not implemented. |

...
test('runtime root exposes all animated assemblies directly, with unique explosion targets',async()=>{const f=readFileSync(new URL('../public/cabinet.glb',import.meta.url));const {scene}=await new GLTFLoader().parseAsync(f.buffer.slice(f.byteOffset,f.byteOffset+f.byteLength),'');const root=cabinetRoot(scene);assert.equal(root.children.length,13);for(const part of root.children)assert.ok(offsets[part.name],part.name);assert.ok(root.children.some(x=>x.name==='crt'));assert.ok(root.children.some(x=>x.name==='logic'));assert.notEqual(offsets.left_panel[0],offsets.right_panel[0]);});

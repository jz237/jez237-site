...
export class DefenderShow extends ArcadeGame {
 ...
 groundY(x:number){return 625-Math.abs(Math.sin(x*.006))*35-Math.abs(Math.sin(x*.017+1))*18;}
 ...
 for(const e of this.foes){if(!e.alive||e===this.mission?.lander)continue;e.x=this.wrap(e.x+(e.kind===1?-this.dir*95:Math.sin(e.phase)*28)*dt);e.y+=Math.sin(this.time*1.6+e.phase)*22*dt;e.y=Math.max(130,Math.min(560,e.y));}
 for(const l of this.lasers){l.x=this.wrap(l.x+l.dir*1400*dt);l.life-=dt;for(const e of this.foes){const distance=Math.abs(((e.x-l.x+this.world*1.5)%this.world)-this.world*.5);if(e.alive&&(e!==this.mission?.lander||this.mission.phase==='intercept')&&distance<38&&Math.abs(e.y-l.y)<26){e.alive=false;l.life=0;if(e===this.mission?.lander){this.rescueEvents.dropped++;this.mission.vy=25;this.rescuePhase('fall');}this.score+=150;this.kills++;this.burst(e.x,e.y,['#74ff85','#ff557e','#d37aff','#ffcc55'][e.kind]);break}}}
 ...
 c.shadowBlur=6;c.shadowColor='#baffb0';for(const [i,x] of this.demoHumans.entries()){if(this.mission&&this.mission.humanIndex===i&&this.mission.phase!=='descend')continue;const a=screen(x);if(a<0||a>w)continue;this.sprite(['.Y.','.W.','WWW','.W.','W.W'],a-4.5,this.groundY(x)-15,3,{Y:'#ffe3a1',W:'#b5ff86'})}
 ...
 const rescue=this.mission;if(rescue&&['lift','intercept','fall'].includes(rescue.phase)){const hx=screen(rescue.x);c.shadowColor='#eeff9e';c.shadowBlur=12;const falling=rescue.phase==='fall';this.sprite(falling&&Math.floor(this.time*9)%2===0?['..Y..','W.W.W','.WWW.','..W..','.W.W.']:['..Y..','..W..','.WWW.','..W..','.W.W.'],hx-5,rescue.y,3,{Y:'#fff2a7',W:'#d5ff95'});if(!falling){c.strokeStyle='#93ff9366';c.lineWidth=1;c.beginPath();c.moveTo(hx,rescue.lander.y+8);c.lineTo(hx,rescue.y);c.stroke();}else{c.strokeStyle='#ffe19844';c.beginPath();c.moveTo(hx-8,rescue.y-20);c.lineTo(hx-8,rescue.y-8);c.moveTo(hx+8,rescue.y-25);c.lineTo(hx+8,rescue.y-13);c.stroke();}}
 ...
 if(this.carrying){c.shadowColor='#baff7b';this.sprite(['.Y.','.W.','WWW','.W.','W.W'],x-4.5,this.y+15,3,{Y:'#fff1af',W:'#aaff80'});c.fillStyle='#cfff8d';c.font='bold 16px monospace';c.fillText('500',x+28,this.y+30)}c.restore();
 ...
 c.shadowColor='#69dfff';c.shadowBlur=7;c.font='bold 26px monospace';c.fillStyle='#e2ffff';c.fillText(String(this.score).padStart(7,'0'),24,43);c.font='bold italic 26px monospace';c.fillStyle='#ffca58';c.fillText('DEFENDER',754,43);c.font='12px monospace';c.fillStyle='#819fb7';c.fillText('1UP  /  AUTOPILOT',24,66);c.fillText('WAVE '+String(this.wave).padStart(2,'0'),788,66);c.font='14px monospace';c.fillStyle='#98bdd0';c.fillText('DEFEND THE HUMANOIDS',26,700);c.fillStyle='#b4ff9a';c.fillText(this.mission?({descend:'LANDER APPROACHING',lift:'HUMANOID ABDUCTED',intercept:'SHOOT THE LANDER',fall:'CATCH THE HUMANOID',return:'HUMANOID CAUGHT'}[this.mission.phase]):'PLANET PATROL',365,700);c.fillStyle='#f9bf67';c.fillText('ATTRACT MODE',789,700);c.shadowBlur=0;

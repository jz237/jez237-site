...
export class DefenderShow extends ArcadeGame {
 ...
 killFoe(e:Foe,quiet=false){
  ...
  if(e===this.mission?.lander){this.rescueEvents.dropped++;this.mission.vy=25;this.rescuePhase('fall');}
 ...
 completeWave(){this.waveBonus=(10-this.deadHumans.size)*100*Math.min(this.wave,5);this.score+=this.waveBonus;this.awardExtra();this.stage='wave';this.stageTimer=3;this.keys.clear();}
 advanceStage(dt:number){
  ...
  this.looseHumans=[];this.x=this.wrap(this.x+480);this.y=300;this.invincible=2;this.mines=[];this.charges=[];this.waveClock=0;this.nextRescue=this.time+3;this.stage='battle';this.effect('start');
 }
 ...
 this.time+=dt;this.invincible=Math.max(0,this.invincible-dt);this.bombClock=Math.max(0,this.bombClock-dt);if(this.stage!=='battle'){this.advanceStage(dt);this.draw();return;}this.checkPlanet();this.flash=Math.max(0,this.flash-dt*2);this.rescueClock+=dt;this.spawnClock+=dt;this.waveClock+=dt;this.escapeClock=Math.max(0,this.escapeClock-dt);
 ...
 const oldY=this.y,oldX=this.x,oldDir=this.dir;if(!this.mission){this.dir=Math.floor(this.time/38)%2===0?1:-1;if(this.planet&&this.time>=this.nextRescue)this.beginRescue();}
 ...
 if(e.kind===0){
  if(e.captive!==undefined){e.y-=55*dt;if(e.y<=125){this.deadHumans.add(e.captive);e.captive=undefined;e.kind=1;this.checkPlanet();}}
   ...
   const h=available[0];if(h){e.target=h.i;e.x=this.wrap(e.x+Math.max(-55*dt,Math.min(55*dt,h.d)));e.y+=Math.sign(this.groundY(e.x)-39-e.y)*28*dt;if(Math.abs(h.d)<12&&e.y>=this.groundY(e.x)-40){e.captive=h.i;e.target=undefined;this.rescueEvents.pickedUp++;if(this.visible(e.x))this.effect('abduct');}}
 ...
 for(const l of this.lasers){l.x=this.wrap(l.x+l.dir*1400*dt);l.life-=dt;for(const e of this.foes){const distance=Math.abs(((e.x-l.x+this.world*1.5)%this.world)-this.world*.5);if(e.alive&&(e!==this.mission?.lander||this.mission.phase==='intercept')&&distance<38&&Math.abs(e.y-l.y)<26){l.life=0;this.killFoe(e);break}}}
 ...
 if(!this.foes.length&&!this.mission){this.completeWave();}
 ...
 else if(!this.mission&&danger.length>=2&&this.escapeClock===0){this.x=this.wrap(this.x+this.dir*(500+this.rand()*1000));this.y=210+this.rand()*230;this.escapeClock=8;this.effect('hyperspace');if(this.rand()>192/256)this.loseShip();else this.invincible=.8;}
 ...
 for(const [i,hx] of this.demoHumans.entries()){if(this.deadHumans.has(i))continue;c.fillStyle='#76ab36';c.fillRect(Math.round(radar(hx)),31,1,1);}

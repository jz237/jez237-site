...
export class DefenderSound {
 ...
 constructor(context:AudioContext){this.context=context;this.master=context.createGain();this.master.gain.value=0;this.analyser=context.createAnalyser();this.analyser.fftSize=256;this.spatialGain=context.createGain();this.spatialGain.gain.value=0;this.pan=context.createStereoPanner();this.lowpass=context.createBiquadFilter();this.lowpass.type='lowpass';this.lowpass.frequency.value=15000;this.master.connect(this.lowpass).connect(this.spatialGain).connect(this.pan).connect(this.analyser).connect(context.destination);this.thrustGain=context.createGain();this.thrustGain.gain.value=0;this.thrustGain.connect(this.master);

...
export function makeRoom(wood:T.Texture,_marquee:T.Texture,_screen:T.Texture){
 const root=new T.Group();root.name='after_hours_arcade';const surfaces=roomSurfaces();
 const steel=new T.MeshStandardMaterial({color:'#7b7971',metalness:.72,roughness:.52,map:surfaces.metal,bumpMap:surfaces.metal,bumpScale:.001});
 const black=new T.MeshStandardMaterial({color:'#262923',roughness:.8,map:wood});
 const woodSide=new T.MeshStandardMaterial({color:'#7e7a69',roughness:.94,map:wood,envMapIntensity:.12});
 const wall=new T.MeshStandardMaterial({color:'#353630',roughness:.97,map:surfaces.wall,bumpMap:surfaces.wall,bumpScale:.015});
 const bricks=['#24272a','#292a2a','#2d2c29','#22272a'].map(color=>new T.MeshStandardMaterial({color,roughness:.96,map:surfaces.wall,bumpMap:surfaces.wall,bumpScale:.01}));
 ...
 box(root,[14,.14,14],[0,5.15,1.4],dark);box(root,[14,5.2,.16],[0,2.6,7.8],wall);box(root,[17,5.2,.16],[0,2.6,-5.6],wall);box(root,[.16,5.2,18],[-7,2.6,0],wall);box(root,[.16,5.2,18],[7,2.6,0],wall);
 ...
 for(let i=0;i<locations.length;i++){
  const [x,z,rot]=locations[i];const g=new T.Group();g.position.set(x,0,z);g.rotation.y=rot;g.scale.set(.9,.86+(i%3)*.035,.9);root.add(g);
 ...
 // Recesses and seams give neighboring machines depth instead of flat rectangular fronts.
 const shadowMat=new T.MeshBasicMaterial({map:surfaces.contact,transparent:true,depthWrite:false,toneMapped:false});
 const contact=(x:number,z:number,w:number,d:number)=>{const o=mesh(root,new T.PlaneGeometry(w,d),shadowMat,[x,.027,z],[-Math.PI/2,0,0]);o.castShadow=false;o.receiveShadow=false;};
 contact(0,0,2.8,2.55);for(const [x,z]of locations)contact(x,z,2,1.8);contact(3.75,.1,1.7,1.2);
 // A workbench with drawer runners, hardware trays, a meter and a task lamp.
 const bench=new T.Group();bench.position.set(2.2,0,-4.6);root.add(bench);
 box(bench,[1.65,.085,.72],[0,1.02,0],woodSide);for(const x of [-.69,.69])for(const z of [-.26,.26])box(bench,[.045,.96,.045],[x,.5,z],steel);
 box(bench,[1.38,.04,.52],[0,.23,0],black);for(const x of [-.43,0,.43]){box(bench,[.4,.18,.49],[x,.85,0],black);box(bench,[.13,.025,.025],[x,.85,.27],steel);}
 const meter=new T.Group();meter.position.set(-.36,1.075,0);meter.rotation.x=-.12;bench.add(meter);box(meter,[.22,.31,.07],[0,.155,0],rubber);mesh(meter,new T.PlaneGeometry(.16,.09),new T.MeshStandardMaterial({map:sign('V / DC','#cbc5a0'),roughness:.7}),[0,.23,.04]);cyl(meter,.035,.02,[0,.12,.045],steel,[Math.PI/2,0,0]);
 for(let i=0;i<8;i++)box(bench,[.075,.035,.10],[.08+(i%4)*.13,1.077,-.12+Math.floor(i/4)*.14],steel);
 const lampPoints=[new T.Vector3(.56,1.07,-.2),new T.Vector3(.57,1.52,-.2),new T.Vector3(.23,1.68,-.04)];mesh(bench,new T.TubeGeometry(new T.CatmullRomCurve3(lampPoints),12,.018,8,false),steel,[0,0,0]);cyl(bench,.12,.025,[.56,1.074,-.2],dark);mesh(bench,new T.ConeGeometry(.15,.13,24,1,true),black,[.23,1.64,-.04]);
 // Surface-mounted conduit, outlet boxes and door jambs.
 for(const x of [-5.7,1.7,5.8]){cyl(root,.013,3.65,[x,2.05,-5.32],steel);box(root,[.17,.21,.065],[x,.57,-5.32],black);for(const y of [1,2,3,3.75])box(root,[.065,.018,.03],[x,y,-5.31],steel);}
 for(const x of [4.28,5.72])box(root,[.075,2.62,.075],[x,1.31,-5.35],steel);box(root,[1.5,.08,.075],[5,2.63,-5.35],steel);
 ...
 for(const [material,items]of bins){const geos=items.map(o=>{const g=o.geometry.clone().applyMatrix4(o.matrixWorld);return g.index?g.toNonIndexed():g;});const g=mergeGeometries(geos,false);if(g){const o=new T.Mesh(g,material);o.castShadow=items.some(item=>item.castShadow);o.receiveShadow=items.some(item=>item.receiveShadow);mergedRoot.add(o);}geos.forEach(g=>g.dispose());items.forEach(o=>o.geometry.dispose());}
 return mergedRoot;
}

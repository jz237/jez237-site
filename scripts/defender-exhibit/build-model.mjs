...
for(const machine of ['defender','robotron','joust']){
const root=buildDetailedCabinet(machine==='robotron',machine==='joust');
mkdirSync('public',{recursive:true});
const glb=await new GLTFExporter().parseAsync(root,{binary:true,onlyVisible:true});
await MeshoptEncoder.ready;await MeshoptDecoder.ready;const io=new NodeIO().registerExtensions(ALL_EXTENSIONS).registerDependencies({'meshopt.encoder':MeshoptEncoder,'meshopt.decoder':MeshoptDecoder});const doc=await io.readBinary(new Uint8Array(glb));await doc.transform(dedup(),weld(),reorder({encoder:MeshoptEncoder,target:'size'}));doc.createExtension(EXTMeshoptCompression).setRequired(true).setEncoderOptions({method:EXTMeshoptCompression.EncoderMethod.FILTER});const optimized=await io.writeBinary(doc);writeFileSync(machine==='defender'?'public/cabinet.glb':`public/${machine}-cabinet.glb`,optimized);

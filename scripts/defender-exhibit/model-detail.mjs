...
export function buildDetailedCabinet(robotron=false,joust=false){
 ...
 if(robotron){m.blue.color.set('#123ab5');m.blue.metalness=0;m.blue.roughness=.45;}
 if(joust){m.blue.color.set('#59adb9');m.amber.color.set('#e5cb3c');}
 ...
 const addStick=(x,name,z=.55)=>{const joy=new T.Group();joy.name=name;joy.position.set(x,1.746,z);cp.add(joy);cyl(cp,.051,.010,[x,1.75,z],m.trim);cyl(joy,.009,.14,[0,.07,0],m.steel);mesh(joy,new T.SphereGeometry(.047,24,16),(joust?(x<0?m.amber:m.blue):m.red),[0,.15,0]);box(cp,[.14,.015,.14],[x,1.59,z],m.zinc);for(let i=0;i<(joust?2:4);i++){const a=joust?i*Math.PI:i*Math.PI/2;const leaf=box(cp,[.07,.004,.022],[x+Math.cos(a)*.075,1.565,z+Math.sin(a)*.075],m.brass,0,'stick_leaf_'+name+'_'+i);leaf.rotation.y=-a;tube(cp,[[x,1.56,z],[x+.03,1.45,.49],[x-.02,1.39,.3]],.004,m.yellowwire,10);}};
 if(joust){addStick(-.43,'joystick',.600);addStick(.264,'joystick_fire',.600);}else if(robotron){addStick(-.264,'joystick',.634);addStick(.264,'joystick_fire',.634);}else{const joystick=new T.Group();joystick.name='joystick';joystick.position.set(-.48,1.746,.57);cp.add(joystick);box(cp,[.052,.012,.155],[-.48,1.749,.57],m.trim,.006);box(cp,[.025,.008,.12],[-.48,1.758,.57],m.ink,.003);cyl(joystick,.009,.14,[0,.07,0],m.steel);mesh(joystick,new T.SphereGeometry(.044,24,16),m.red,[0,.15,0]);}

 for(const [i,x,z,color]of (joust?[[2,-.244,.600,m.amber],[3,.427,.600,m.blue],[5,-.080,.466,m.red],[6,.083,.466,m.red]]:robotron?[[5,.516,.517,m.blue],[6,.516,.732,m.blue]]:[[0,-.36,.73,m.ivory],[1,.32,.49,m.ivory],[2,.46,.49,m.ivory],[3,.24,.65,m.green],[4,0,.80,m.ivory],[5,-.12,.46,m.red],[6,.12,.46,m.red]])){cyl(cp,.043,.012,[x,1.746,z],color);ring(cp,.035,.002,[x,1.756,z],m.trim,[Math.PI/2,0,0]);mesh(cp,new T.LatheGeometry([[0,-.001],[.008,-.0005],[.018,.001],[.025,.004],[.029,.009],[.031,.010],[.032,.007],[.032,-.012],[0,-.012]].map(([r,y])=>new T.Vector2(r,y)),32),color,[x,1.769,z],'button_'+i);cyl(cp,.034,.12,[x,1.61,z],m.ivory);box(cp,[.1,.014,.055],[x,1.54,z],m.ink);box(cp,[.11,.004,.025],[x,1.515,z],m.brass,0,'leaf_'+i);box(cp,[.11,.004,.025],[x,1.507,z],m.brass,0);cyl(cp,.008,.016,[x+.033,1.511,z],m.steel);tube(cp,[[x,1.53,z-.022],[x+.03,1.44,z-.04],[x-.02,1.39,.3]],.004,m.yellowwire,10);}
 for(const x of [-.61,.61])for(const z of [.39,.83])screw(cp,[x,1.752,z],'y');if(!robotron&&!joust){box(cp,[.12,.08,.2],[-.48,1.57,.57],m.black);for(const z of [.485,.655])box(cp,[.1,.008,.026],[-.48,1.532,z],m.brass);}
 box(cp,[1.34,.14,.025],[0,1.6,.884],m.black);if(robotron||joust)mesh(cp,new T.PlaneGeometry(1.29,.178),mat('control_apron_print','#fff',0,.8),[0,1.622,.910],'control_apron_print');
 const coin=part('coin');const doorFrame=new T.Shape();doorFrame.moveTo(-.645,.445);doorFrame.lineTo(.645,.445);doorFrame.lineTo(.645,1.545);doorFrame.lineTo(-.645,1.545);doorFrame.closePath();const doorOpening=new T.Path();doorOpening.moveTo(-.365,.56);doorOpening.lineTo(-.365,1.44);doorOpening.lineTo(.365,1.44);doorOpening.lineTo(.365,.56);doorOpening.closePath();doorFrame.holes.push(doorOpening);mesh(coin,new T.ExtrudeGeometry(doorFrame,{depth:.062,bevelEnabled:false}),m.wood,[0,0,.502],'coin_fascia');box(coin,[.72,.87,.025],[0,1,.58],m.zinc,.018);box(coin,[.674,.826,.034],[0,1,.601],m.trim,.024);box(coin,[.622,.777,.022],[0,1,.625],m.black,.02);
 for(const x of [-.158,.158]){box(coin,[.214,.258,.033],[x,1.203,.652],m.zinc,.016);box(coin,[.182,.228,.037],[x,1.203,.672],m.trim,.012);box(coin,[.080,.136,.012],[x+.027,1.232,.697],mat('coin_lamp','#b64214',.1,.58),.006,'coin_lamp');box(coin,[.014,.110,.012],[x-.049,1.232,.707],m.ink,.002);box(coin,[.075,.037,.019],[x,1.131,.702],m.black,.005);box(coin,[.011,.28,.19],[x-.09,1.095,.4],m.zinc);box(coin,[.011,.29,.19],[x+.075,1.095,.4],m.zinc);for(let i=0;i<4;i++)cyl(coin,.023,.14,[x,1.21-i*.047,.36],m.steel,[0,0,Math.PI/2]);tube(coin,[[x,1.1,.3],[x,.94,.34],[x,.75,.25]],.023,m.zinc,12);}
 box(coin,[.34,.14,.035],[0,.795,.663],m.zinc,.01);box(coin,[.304,.112,.042],[0,.795,.69],m.ink,.008);box(coin,[.28,.033,.035],[0,.75,.71],m.black);cyl(coin,.025,.018,[.244,1,.654],m.brass,[Math.PI/2,0,0]);box(coin,[.005,.026,.008],[.244,1,.668],m.ink,0);rivetGrid(coin,[-.29,.29],[.68,1.326],.657);box(coin,[.48,.16,.34],[0,.57,.22],m.zinc,.007,'coin_cashbox');box(coin,[.435,.015,.293],[0,.658,.22],m.ink,.007,'coin_cashbox_opening');cyl(coin,.045,.006,[.158,1.23,.37],m.brass,[Math.PI/2,0,0],'travelling_coin');
 const {logic:boards,power}=addHardware({root,part,box,cyl,tube,ring,screw,m,mesh,mat,layouts:joust?joustLayouts:robotron?robotronLayouts:undefined});
 const speaker=part('speaker');box(speaker,[1.27,.055,.39],[0,2.807,.27],m.wood);const speakerPlate=new T.Shape();speakerPlate.moveTo(-.215,-.2025);speakerPlate.lineTo(.215,-.2025);speakerPlate.lineTo(.215,.2025);speakerPlate.lineTo(-.215,.2025);speakerPlate.closePath();const coneOpening=new T.Path();coneOpening.absarc(0,0,.151,0,Math.PI*2,true);speakerPlate.holes.push(coneOpening);mesh(speaker,new T.ExtrudeGeometry(speakerPlate,{depth:.018,bevelEnabled:false,curveSegments:32}),m.zinc,[0,2.817,.471]);const conePoints=[[.014,-.046],[.058,-.04],[.142,.002],[.154,.007],[.167,.005]].map(([r,z])=>new T.Vector2(r,z));const conePaper=mat('speaker_paper','#282724',0,.96);conePaper.side=T.DoubleSide;const cone=mesh(speaker,new T.LatheGeometry(conePoints,48),conePaper,[0,2.817,.523],'speaker_cone');cone.rotation.x=Math.PI/2;for(const r of [.149,.157,.165])ring(speaker,r,.004,[0,2.817,.523],m.trim);mesh(speaker,new T.SphereGeometry(.05,24,12),m.trim,[0,2.817,.534]);for(const x of [-.185,.185])for(const y of [2.64,2.99])screw(speaker,[x,y,.494]);for(let i=0;i<8;i++){const a=i*Math.PI/4;tube(speaker,[[Math.cos(a)*.157,2.817+Math.sin(a)*.157,.474],[Math.cos(a)*.081,2.817+Math.sin(a)*.081,.345]],.008,m.zinc,4);}cyl(speaker,.077,.074,[0,2.817,.315],m.black,[Math.PI/2,0,0]);
 ...
 addFinish({robotron:robotron||joust,joust,root,cp,coin,crt,marquee,frame,back,box,cyl,ring,tube,mesh,screw,m,mat});
 // Clear the bezel overhang while retaining assembly-local animation pivots.
 for(const o of cp.children)o.position.z+=.18;
 // Service hinges keep their physical attachment points fixed as panels move.
 const hinge=(assembly,name,point,keep=[])=>{const pivot=new T.Group();pivot.name=name;pivot.position.set(...point);for(const o of [...assembly.children])if(!keep.includes(o)){o.position.sub(pivot.position);pivot.add(o);}assembly.add(pivot);return pivot;};
 hinge(coin,'coin_hinge',[-.31,1,.65],coin.children.filter(o=>o.name==='coin_fascia'||o.name.startsWith('coin_cashbox')));
 hinge(cp,'controls_hinge',[0,1.62,.525]);
 hinge(back,'rear_service',[0,1.77,-.73]);
 hinge(vents,'rear_vents',[0,1.77,-.73]);

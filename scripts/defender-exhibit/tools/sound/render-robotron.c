...
static uint8_t rom[4096],ram[128],pia[4],command=255,dac=128;
uint8_t m6800_read(struct m6800*c,uint16_t a){if(a>=0xf000)return rom[a-0xf000];if(a<128)return ram[a];if(a==0x402){m6800_clear_interrupt(c,IRQ_IRQ1);return command;}if(a>=0x400&&a<0x404)return pia[a-0x400];return 255;}

export type FishPoint={id:number;x:number;y:number;vx?:number;vy?:number};
...
export function thinkFish(b:FishBrain,dt:number,x:number,y:number,speed:number,senses:FishSenses){
 ...
 const food=senses.food.filter(f=>Math.hypot(f.x-x,f.y-y)<330).sort((a,c)=>Math.hypot(a.x-x,a.y-y)-Math.hypot(c.x-x,c.y-y))[0];
 if(food&&b.hunger>.18&&Math.hypot(food.x-x,food.y-y)<22&&b.biteIn===0){b.consumedFood=food.id;b.hunger=clamp(b.hunger-.23);b.energy=clamp(b.energy+.08);b.biteIn=1.2;b.decisionIn=0;}
 const close=senses.neighbors.filter(f=>Math.hypot(f.x-x,f.y-y)<26).sort((a,c)=>Math.hypot(a.x-x,a.y-y)-Math.hypot(c.x-x,c.y-y))[0];
 ...
 if(close)b.intent={kind:'space',reason:'Giving a nearby fish more room.',target:{id:-1,x:x+(x>=close.x?65:-65),y:y+(y>=close.y?22:-22)}};
  ...
  const school=senses.neighbors.filter(f=>Math.hypot(f.x-x,f.y-y)<260&&Math.hypot(f.x-x,f.y-y)>45);
  if(school.length>=2&&random(b)<.48){const count=school.length;b.intent={kind:'school',reason:`Following ${count} nearby tetras.`,target:{id:-2,x:school.reduce((v,f)=>v+f.x,0)/count,y:school.reduce((v,f)=>v+f.y,0)/count}};b.decisionIn=4+random(b)*3;}

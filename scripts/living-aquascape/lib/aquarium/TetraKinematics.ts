...
export function tetraSpine(x:number,phase:number,activity:number){
 ...
 const amplitude=.002+effort*.115,travel=phase-u*3.3;
 const envelope=Math.pow(u,1.65);
 const z=amplitude*envelope*Math.sin(travel);
 const slope=u>0?-amplitude/.65*(1.65*Math.pow(u,.65)*Math.sin(travel)-3.3*envelope*Math.cos(travel)):0;

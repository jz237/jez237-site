...
export class PhotographicScene{
 ...
 readonly camera=new T.OrthographicCamera(-W/2,W/2,H/2,-H/2,-100,100);
...
#include <colorspace_fragment>
 ...
 if(i===0){advanceTetraSwim(this.tetraSwim,moveDt,this.feeding>0,s.oxygen<4,{food:this.food.children.map(o=>({id:o.id,x:o.position.x+W/2,y:H/2-o.position.y})),neighbors:this.fish.slice(1).filter(n=>n.species===0).map((n,id)=>({id,x:n.x,y:n.y,vx:n.vx,vy:n.vy}))});const eaten=this.food.children.find(o=>o.id===this.tetraSwim.brain.consumedFood);if(eaten instanceof T.Mesh){this.food.remove(eaten);eaten.geometry.dispose();(eaten.material as T.Material).dispose();}this.tetraSwim.brain.consumedFood=null;const swim=this.tetraSwim;f.x=swim.x;f.y=swim.y;f.vx=swim.vx;f.vy=swim.vy;f.turn=swim.yaw;}else {const desired=f.vx>=0?0:Math.PI;f.turn+=(desired-f.turn)*(1-Math.exp(-moveDt*2.4));}

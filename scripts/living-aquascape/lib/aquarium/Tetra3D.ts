...
export class Tetra3D {
 ...
 constructor(texture:T.Texture){
  ...
  const membrane=(points:number[][])=>{const positions:number[]=[],coords:number[]=[],indices:number[]=[];for(const [x,y,z=0] of points){positions.push(x,y,z);coords.push(x+.5,.5+y/.45);}for(let i=1;i<points.length-1;i++)indices.push(0,i,i+1);const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(positions,3));g.setAttribute('uv',new T.Float32BufferAttribute(coords,2));g.setIndex(indices);g.computeVertexNormals();this.add(g,fin);};
  membrane([[-.30,-.035],[-.495,.12],[-.43,.015],[-.395,-.035],[-.43,-.09],[-.495,-.185]]);
  membrane([[-.10,.056],[-.065,.22],[.10,.07]]);
  membrane([[-.23,-.079],[-.12,-.21],[.005,-.141],[.12,-.13]]);
  for(const side of [-1,1])membrane([[.25,-.055,side*.049],[.05,-.17,side*.10],[.18,-.10,side*.022]]);
 ...
 private add(geometry:T.BufferGeometry,material:T.MeshPhysicalMaterial){const mesh=new T.Mesh(geometry,material);this.meshes.push(mesh);this.originals.set(geometry,new Float32Array(geometry.getAttribute('position').array));this.group.add(mesh);}
 update(time:number,activity:number,photo:T.Texture,flow:number,depth:number,daylight:number){
  for(const mesh of this.meshes){const p=mesh.geometry.getAttribute('position') as T.BufferAttribute,rest=this.originals.get(mesh.geometry)!;for(let i=0;i<p.count;i++){const x=rest[i*3],y=rest[i*3+1],z=rest[i*3+2],tail=Math.pow(Math.max(0,(.32-x)/.82),2);p.setXYZ(i,x,y,z+Math.sin(time*(5+activity*3)-x*5)*tail*.068);}p.needsUpdate=true;mesh.geometry.computeVertexNormals();}

...
const registered=sectionEngine(engine,plane,new Set(shell.material));
assert.equal(registered.size,2);
for(const material of shell.material)assert.equal(material.clippingPlanes[0],plane);
assert.equal(shell.customDepthMaterial.clippingPlanes[0],plane);
assert.equal(particles.material.clippingPlanes,null);
...
console.log('Verified housing-only reveal registration, material arrays, window shader, shadow clipping and helper exclusions.');

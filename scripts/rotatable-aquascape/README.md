...
### Desktop capture scheduling (2026-09-15)

Full-resolution shadow/reflection scheduling now defaults on for desktop as well as phone input. A mouse is not evidence of a fast GPU. The existing oldest-first scheduler services one secondary view per frame, preserves paired reflection projection/image data, and refreshes new views and camera/lesson jumps immediately. Main-view geometry, pixel ratio, textures, anti-aliasing, AO, lighting resolution, and simulation/body/fin updates are unchanged. Secondary images can be a few frames old; this trades their refresh frequency for smoother main-view motion. `?captures=full` retains the previous every-frame capture path for comparisons.

Refraction gathers material visibility once after simulation/lesson updates and shares it across camera passes. It rebuilds this list each frame so food and learning geometry are included, preserves hidden materials, and restores visibility even when rendering fails. Tests cover independent cameras, dynamic contents, full-resolution copies, and failure recovery. The diagnostic action is now named Run graphics test on all devices.

At an identical 1920 x 892 drawing buffer, local development measurements changed from 1,412 draw calls / 65,685,372 submitted triangles to 420 / 18,950,829 (about 70% / 71% fewer). These count repeated passes, not removed model detail. GPU medians were about 8.08 ms before and 4.36 ms after; render CPU medians about 4.0 ms and 2.0 ms. This desktop was near the 60 Hz presentation limit in both configurations, so these measurements demonstrate reduced work and increased headroom, not a universal FPS multiplier. A larger desktop viewport, angled reflections, feeding and root isolation were visually checked. All 157 aquarium tests pass.

### Further CPU/GPU reuse and adaptive capture cadence (2026-09-15)

Fish surface normals use fixed-layout array arithmetic with byte-for-byte agreement against Three's area-weighted normal calculation over 40 swimming poses. Unchanged paused poses avoid duplicate uploads. Shrimp articulation composes matrices directly instead of round-tripping every leg/antenna rotation through Object3D Euler callbacks; all instance matrices match the previous path. Conservative fish sweep bounds reject distant grazer contacts, and repeated body construction and plant-current calculations are shared. Detailed nearby collision tests, whole-body waves, fins, feeding and navigation remain active. The bottom-feeder route cache was rebuilt for the current contact implementation.

Plant shaders reuse the same deformed position across projection, lighting and shadows. Baked-light world-position reconstruction avoids redundant normalization. The MIT-licensed meshoptimizer encoder changes only opaque scanned-hardscape triangle order: every oriented face and original attribute remains intact. Vertex-cache models show about 58% fewer repeated wood vertex evaluations at 32 entries, and up to about 76% for individual rock scans. These modeled cache savings are not whole-frame GPU speedups. Unsupported optimization falls back to the original geometry.

Sustained slow presentation now switches secondary captures to alternate frames. Every active shadow/reflection map still receives bounded oldest-first service; new views and camera/lesson jumps refresh immediately. Recovery uses a longer stable window to avoid rapid mode changes. Main-view resolution, textures, geometry, AO and animal simulation are unchanged. Secondary images can be older in adaptive mode; this is a temporal lighting tradeoff. Diagnostic URLs can fix the schedule with `?captures=staggered`, `?captures=alternate`, or `?captures=full`. `?originalHardscapeIndices` and `?originalPlantShader` isolate the draw-order and shader changes.

In local production comparisons at the same 1920 x 892 drawing buffer, simulation medians changed from 4.8 ms to about 3.0 ms, while GPU medians changed from 4.53 ms to 4.25 ms with the original staggered cadence and about 3.91 ms with alternate captures forced. The computer remained near its 60 Hz presentation cap. Actual affected-desktop measurements are still needed; these results do not guarantee the same improvement on every GPU. Automatic cadence transitions, recovery and map fairness are covered by deterministic tests; the fast local desktop naturally stays at the original cadence. Angled/day/night views, feeding, pause and the root lesson were checked, with no browser shader errors. All 167 tests pass, including 4,000 sweep-contact equivalence cases and complete planted-tank animal routing.

### Indirect-light bandwidth and affected-desktop diagnostics (2026-09-15)

A reported desktop measured 9.7 FPS at 1920 x 795, 14.9 FPS with simulation and secondary captures held, and 16.7 FPS with simulation held and half image dimensions. These independent comparisons indicate substantial rendering cost remains; they do not identify the graphics device or prove software rendering. Simulation measured 11 ms and is not enough by itself to explain the reported roughly 100 ms frames.

The L1 bounced-light field now packs each color channel's four directional coefficients into one RGBA texel. Three filtered texture reads replace four, with 25% less field storage. Every coefficient retains the same half-float value and trilinear filtering; normal-dependent, per-pixel lighting, all geometry, textures and animation stay intact. Tests compare all real field coefficients and interpolated boundary/interior samples. The original bake files remain unchanged. This is a 25% reduction in this field's texture work, not a whole-frame speedup claim.

The opt-in local performance readout now includes the graphics renderer exposed by the browser, with a fallback when unavailable. It is not sent to a server. Two additional independent probes combine held captures with contact shading bypass or half image dimensions to better isolate main-view cost. The complete test takes about 50 seconds and restores resolution and controls on completion or cancellation. All 169 tests pass. The local RTX 5090 preview holds 60 FPS, which does not establish performance on the affected desktop.

### Automatic effects for slower computers (2026-09-15)

Following the reported Radeon R9 200 Series results (14.4 FPS full, 25 FPS at half image dimensions, little benefit from holding simulation), effects now adapt to measured sustained frame times. Every ordinary visit starts with full effects; no GPU-name, memory-size, desktop/mobile or input-device rule lowers quality. Stable 30 Hz presentation retains full effects. Windows longer than about 2.4 seconds must have a median above 38 ms and a lower quartile above 32 ms before moving down one level, following a startup/transition warmup.

The first fallback halves only contact-shading target dimensions and keeps the kernel's final-image footprint. The second disables that shading and uses a single hardware-filtered shadow comparison plus planar distorted water reflections in place of the 28-step depth-guided search. Surface ripples, Fresnel response, all lights and shadow casters remain. The last fallback also disables MSAA and reduces reflection-image dimensions to 65%, with secondary captures spaced over four frames. The main drawing-buffer dimensions, source textures, geometry, animal populations, body/fin movement, feeding, schooling and collisions are unchanged.

Recovery restores one level after sustained fast frames, with an increasing delay after a failed recovery to avoid repeated visual switching. Hidden tabs, separate filter views, paused scenes, isolated lessons and graphics benchmarks do not train the effects controller. The Effects selector shows the active automatic level; Always full restores the complete effect settings and disables further automatic reductions for the current visit. The opt-in readout lists the actual effect reductions. `?effects=0` through `?effects=3` are explicit diagnostic overrides; normal links use automatic mode.

The graphics test temporarily uses full effects for comparable probes and restores the previous effects, resolution and controls afterward. Tests cover fast/30 Hz preservation, slow-frame transitions, recovery/backoff, interruptions, full-size beauty buffers, reversible reflection resources, exact retention of the original full PCF shader, and intact water geometry/ripples. On the local RTX 5090, automatic mode retained full effects at 60 FPS; forced fallback checks are functional tests, not measurements on the reported Radeon computer. Its improvement still needs to be measured there.

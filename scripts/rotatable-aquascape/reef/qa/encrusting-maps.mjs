...
for(const [folder,budget] of [['encrusting',700000],['plate',750000]]){
let bytes=0;
for(const name of ['map','normalMap','roughnessMap']){
 const buffer=fs.readFileSync(new URL(`../assets/${folder}/${name}.png`,import.meta.url));bytes+=buffer.length;
...
assert.ok(bytes<budget,'baked full-resolution coral detail stays within the reviewed download budget');
console.log(folder,'maps passed:',bytes,'bytes; three512px maps.');
}

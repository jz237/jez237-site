...
try{
 ...
 await page.waitForTimeout(1500);const initial=await page.evaluate(()=>window.reefQA.snapshot());assert.equal(initial.fish,21);assert.ok(initial.rockLifeStats.patches>150);assert.ok(initial.rockLifeStats.pores>3000);assert.ok(initial.rockLifeStats.triangles<750000);assert.equal(initial.rockLifeStats.colors,7);assert.equal(initial.canopyStats.colonies,12);assert.ok(initial.canopyStats.triangles>200000&&initial.canopyStats.triangles<910000);assert.equal(initial.rearGlass.visible,true);assert.equal(initial.infillStats.colonies,11);assert.ok(initial.crustStats.understoryColonies>=20,'retain the new layer of smaller coral colonies');assert.ok(initial.crustStats.understoryTriangles<1120000,'merged detailed growth stays within its reviewed geometry budget');assert.ok(initial.infillStats.triangles<540000);assert.ok(initial.polypStats.polyps>500);assert.ok(initial.polypStats.maxAttachmentError<.00301);assert.ok(initial.crustStats.colonies>30&&initial.crustStats.colonies<=100,'exposed rock retains distributed encrusting coverage');assert.ok(initial.crustStats.triangles>1000&&initial.crustStats.triangles-initial.crustStats.mantleTriangles<70000);assert.equal(initial.anemoneTentacles,540);assert.equal(initial.obstacleOverlaps,0);assert.equal(initial.fishOverlaps,0);assert.equal(initial.anatomy.length,21);assert.ok(initial.anatomy.every(f=>f.model.startsWith('Blender')&&f.pectoral.length===2&&f.gills.length===2),'every inhabitant uses the articulated Blender model');
 ...
 assert.ok(initial.crustStats.emergentTriangles>75000&&initial.crustStats.emergentTriangles<140000,'small emergent colonies preserve detailed anatomy within their reviewed budget');

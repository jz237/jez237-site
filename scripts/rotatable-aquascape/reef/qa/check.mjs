...
try{
 ...
 const fed=await page.evaluate(()=>window.reefQA.snapshot());assert.ok(fed.anatomy.every((f,i)=>f.phase!==initial.anatomy[i].phase),'independent body waves advance');assert.notDeepEqual(fed.anatomy.map(f=>[f.pectoral,f.gills,f.mouth]),initial.anatomy.map(f=>[f.pectoral,f.gills,f.mouth]),'fins and breathing must move');assert.ok(fed.anatomy.every(f=>f.pectoral[0][1]*f.pectoral[1][1]<0),'paired pectorals sweep outward on opposite flanks');assert.ok(fed.bites>=12,'fish must physically reach and consume food');assert.ok(fed.positions.some((p,i)=>Math.hypot(p.x-initial.positions[i].x,p.y-initial.positions[i].y,p.z-initial.positions[i].z)>.5),'inhabitants explore rather than staying stuck');

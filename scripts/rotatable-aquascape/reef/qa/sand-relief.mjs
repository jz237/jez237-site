...
for(let ix=0;ix<=300;ix++)for(let iz=0;iz<=150;iz++){
 ...
 assert.ok(h>.15&&h<.64,'bounded low dunes; fish and food use terrain-aware clearance');
...
assert.ok(max-min>.36,'visible static banks');
assert.ok(maxSlope<.9,'no abrupt bed steps or cliffs');

...
let active=0;for(let i=1;i<=800;i++){const length=Math.hypot(...b.brushData.slice(i*4,i*4+3));if(length>.001)active++;if(i>360)assert.equal(length,0,'other hosts must not react to rubbing');}
...
for(let i=0;i<lookup.count;i++){if(flex.getW(i)===0)assert.equal(lookup.getX(i),0);else assert.ok(lookup.getX(i)>=1&&lookup.getX(i)<=800);}

...
container.appendChild(renderer.domElement);const camera=new T.PerspectiveCamera(31,1,.1,120);camera.position.set(0,3.15,17.7);
...
const ambient=new T.HemisphereLight('#b1d5ff','#414b54',2.0);scene.add(ambient);
const key=new T.SpotLight('#d9e4ff',130,26,.91,.52,1.7);key.position.set(-3,8,3.5);key.target.position.set(-1,1,0);key.castShadow=true;key.shadow.mapSize.set(1536,1536);key.shadow.bias=-.00015;key.shadow.normalBias=.018;scene.add(key,key.target);
const blue=new T.SpotLight('#5289ff',110,24,.9,.6,1.7);blue.position.set(3,7.5,-.3);blue.target.position.set(1.5,1,0);scene.add(blue,blue.target);
const fill=new T.DirectionalLight('#96b9f3',1.4);fill.position.set(1,4,7);scene.add(fill);
...
function animate(now:number){requestAnimationFrame(animate);const elapsed=now-last;last=now;if(document.hidden)return;const dt=Math.min(elapsed/1000,.04);
 ...
 const l=night?.42:1;ambient.intensity=T.MathUtils.damp(ambient.intensity,2*l,5,dt);key.intensity=T.MathUtils.damp(key.intensity,130*l,5,dt);blue.intensity=T.MathUtils.damp(blue.intensity,night?135:110,5,dt);fill.intensity=T.MathUtils.damp(fill.intensity,1.4*l,5,dt);water.update(time,camera.position.y,l);
...
// Read-only QA hook: verifies real positions, bites and collision invariants.
Object.assign(window,{reefQA:{snapshot:()=>({...fish.snapshot(),time,paused,ready,triangles,effects:effectProfiles[adaptive.level].name,fps:1000/(sampleMs.reduce((a,b)=>a+b,0)/sampleMs.length),drawCalls:renderer.info.render.calls}),feed:()=>fish.feed()}});

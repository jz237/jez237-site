...
const reef=buildReef(scene,rockLifeAttachments,reefAssets);
...
const drawOrderStats=optimizeReefDrawOrder(reef.group);
performance.measure('reef-draw-order','reef-draw-order-start');
performance.mark('reef-picking-start');
const pickingStats=accelerateReefPicking(reef.group);
...
$('#feed').onclick=()=>{if(paused){paused=false;$('#pause').textContent='Pause';$('#pause').setAttribute('aria-pressed','false');}if(feedReef())$('#status').textContent='Feeding \u00b7 Watch fish bites and sinking anemone morsels';};

...
export function buildReef(scene:T.Scene){
 ...
 const branch=(x:number,y:number,z:number,size:number,hue:number,rng:()=>number=random)=>{
  ...
  const colony=branchingColony(base,size,hue,rng,(px,pz)=>surfaceLookup(px,base.y+.28*size,pz)?.point.y??null,hue>.9?'antler':hue>.7?'canopy':'bushy',support?.face?coralCrust((support.object as T.Mesh).geometry,support.point,support.face.normal,.23*size,hue,hue*17+size,{color:new T.Color().setHSL(hue,.52,.29),thickness:.006*size}):undefined),center=base.clone().add(new T.Vector3(0,size*.47,0));let radiusSquared=0;

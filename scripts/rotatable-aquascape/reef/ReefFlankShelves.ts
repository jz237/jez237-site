...
export function flankShelf(point:T.Vector3,normal:T.Vector3,radius:number,seed:number,hue:number){
 ...
 const positions:number[]=[],colors:number[]=[],uv:number[]=[],tissue:number[]=[],indices:number[]=[];
 ...
 for(let layer=0;layer<2;layer++)for(let j=0;j<=rings;j++)for(let i=0;i<=sides;i++){
  ...
  colors.push(color.r,color.g,color.b);tissue.push(Math.round(255*(layer?.12:1-margin)));
 ...
 const geometry=new T.BufferGeometry();geometry.setAttribute('position',new T.Float32BufferAttribute(positions,3));geometry.setAttribute('color',new T.Float32BufferAttribute(colors,3));geometry.setAttribute('uv',new T.Float32BufferAttribute(uv,2));geometry.setAttribute('plateTissue',new T.Uint8BufferAttribute(tissue,1,true));geometry.setIndex(indices);geometry.computeVertexNormals();geometry.computeBoundingSphere();

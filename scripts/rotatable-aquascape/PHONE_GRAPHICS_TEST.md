...
## Second S25 result and water shader work

The user measured `captures-734c3098f` at 31.7 FPS during normal use and 34.2/34.4 FPS in the full-animation benchmark passes. Both captures held still produced 49.1 FPS; halving width and height produced 51.1 FPS. Frame intervals remained uneven. These comparisons suggest remaining screen-pixel work matters, but the held-capture tests also redistribute the scheduler budget and cannot be read as individual pass costs.

The next revision interleaves full-scene reflections with depth-only shadows instead of clustering three of each. Capture frequency and resolutions are unchanged. Water normals now use analytic derivatives of the same six-wave field, coordinate warp, inlet rings and meniscus rather than four complete height evaluations per fragment. Numerical tests compare the actual scalar GLSL with finite differences of the previous height equations at multiple times, footprint filters, inlet positions and glass edges. The small finite-difference approximation in the old normals is replaced by the actual slope; the underlying wave shape and all frequency components remain intact.

Depth-guided reflection retains 28 search steps and four refinement steps. Ray projection is hoisted outside the loops, and depth reconstruction evaluates only the needed Z and W matrix rows. No image-resolution, geometry, texture, lighting sample or contact-shading reductions are used. Actual S25 gains still require a new device measurement.

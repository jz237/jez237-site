...
test('contact deformation matches the actual plant vertex shader at varied flow times',async()=>{
 ...
 const {Corydoras,coryBody,coryForward}=await import('../lib/Corydoras.ts');const scene=new T.Scene();buildBotanicalPlants(scene,height,{value:0});addFernFixture(scene);calmSwordLeaves(scene);scene.updateMatrixWorld();const grazers=new Invertebrates(scene,height),life=new Corydoras(scene,height,[],grazers.plants),distance=Array(6).fill(0);let plantContacts=0;
 for(let i=0;i<900;i++){const old=life.animals.map(a=>a.position.clone());life.update(.1,i*.1,[],grazers.animals);grazers.externalBodies=life.animals.map(a=>coryBody(a.position,coryForward(a),a.size));grazers.update(.1,i*.1);for(const a of life.animals){distance[a.id]+=a.position.distanceTo(old[a.id]);for(const g of grazers.animals)assert.equal(bodiesOverlap(coryBody(a.position,coryForward(a),a.size),grazerBody(g.position,g.normal,new T.Vector3().setFromMatrixColumn(g.matrix,0).normalize(),g.kind==='snail',g.kind==='shrimp'?.80+g.id%3*.04:.84),0),false,'cory and grazer stay separate');assert.ok(a.position.distanceTo(old[a.id])<.04);if(!grazers.plants.clearBody(a.position,coryBody(a.position,coryForward(a),a.size),i*.1))plantContacts++;for(const b of life.animals)if(b.id>a.id)assert.equal(bodiesOverlap(coryBody(a.position,coryForward(a),a.size),coryBody(b.position,coryForward(b),b.size),0),false);}}
 assert.equal(plantContacts,0,'moving foliage must not penetrate cory envelopes');assert.ok(distance.every(d=>d>3),JSON.stringify(distance));assert.ok(life.animals.every(a=>a.visits.size>=5),'each cory visits at least five floor patches within 90 seconds');grazers.dispose();
 });

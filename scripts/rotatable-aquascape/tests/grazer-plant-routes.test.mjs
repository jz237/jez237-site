...
test('full planting supports climbing, swimming departures, landings and pause without teleporting',()=>{
 ...
 for(let i=0;i<1800;i++){
  ...
  const bodies=life.animals.map(a=>grazerBody(a.position,a.normal,new T.Vector3().setFromMatrixColumn(a.matrix,0).normalize(),a.kind==='snail',a.scale));for(let a=0;a<9;a++)for(let b=a+1;b<9;b++)assert.equal(bodiesOverlap(bodies[a],bodies[b],0),false,`grazer bodies overlap at ${i}: ${a}/${b}`);
...
test('contact deformation matches the actual plant vertex shader at varied flow times',async()=>{
 ...
 const {Corydoras,coryBody,coryForward}=await import('../lib/Corydoras.ts');const scene=new T.Scene();buildBotanicalPlants(scene,height,{value:0});addFernFixture(scene);calmSwordLeaves(scene);scene.updateMatrixWorld();const obstacles=await hardscapeObstacles(),grazers=new Invertebrates(scene,height,[],undefined,obstacles),life=new Corydoras(scene,height,obstacles,grazers.plants),distance=Array(6).fill(0),lateDistance=Array(6).fill(0);await life.prepareNavigation(JSON.parse(fs.readFileSync(new URL('../lib/CoryFloorRoutes.json',import.meta.url),'utf8')));let plantContacts=0,maxLift=0,totalLift=0,gravelPicks=0,leafPicks=0;const bounds=[Infinity,-Infinity,Infinity,-Infinity];
 for(let i=0;i<1800;i++){const old=life.animals.map(a=>a.position.clone());life.update(.1,i*.1,[],grazers.animals);grazers.externalBodies=life.animals.map(a=>coryBody(a.position,coryForward(a),a.size,a.pitch));grazers.update(.1,i*.1);for(const a of life.animals){const lift=a.position.y-height(a.position.x,a.position.z);maxLift=Math.max(maxLift,lift);totalLift+=lift;if(a.picking?.kind==='gravel')gravelPicks++;if(a.picking?.kind==='leaf')leafPicks++;distance[a.id]+=a.position.distanceTo(old[a.id]);if(i>=1200)lateDistance[a.id]+=a.position.distanceTo(old[a.id]);bounds[0]=Math.min(bounds[0],a.position.x);bounds[1]=Math.max(bounds[1],a.position.x);bounds[2]=Math.min(bounds[2],a.position.z);bounds[3]=Math.max(bounds[3],a.position.z);for(const g of grazers.animals)assert.equal(bodiesOverlap(coryBody(a.position,coryForward(a),a.size,a.pitch),grazerBody(g.position,g.normal,new T.Vector3().setFromMatrixColumn(g.matrix,0).normalize(),g.kind==='snail',g.scale),0),false,'cory and grazer stay separate');assert.ok(a.position.distanceTo(old[a.id])<.04);if(!grazers.plants.clearBody(a.position,coryBody(a.position,coryForward(a),a.size,a.pitch),i*.1))plantContacts++;for(const b of life.animals)if(b.id>a.id)assert.equal(bodiesOverlap(coryBody(a.position,coryForward(a),a.size,a.pitch),coryBody(b.position,coryForward(b),b.size,b.pitch),0),false);}}
 assert.ok(maxLift<.65,'stays in the bottom habitat');assert.ok(totalLift/10800<.35,'prefers the substrate');assert.ok(gravelPicks>20&&leafPicks>5,'occasionally searches both gravel and low leaves');assert.equal(plantContacts,0,'moving foliage must not penetrate cory envelopes');assert.ok(distance.every(d=>d>3),JSON.stringify(distance));assert.ok(bounds[0]<-3.5&&bounds[1]>3.5,'the group explores both ends of the tank');assert.ok(lateDistance.every(d=>d>1),'each cory keeps moving in the final minute: '+JSON.stringify(lateDistance));grazers.dispose();
 });

// Reference results recorded from build adaptive-37aef7493 before broad rejection.
test('whole-body plant rejection preserves 4000 detailed moving-surface results',async()=>{
 const {createHash}=await import('node:crypto'),{plantClearanceCases}=await import('./plant-clearance-cases.mjs');
 const scene=new T.Scene();buildBotanicalPlants(scene,height,{value:0});addFernFixture(scene);calmSwordLeaves(scene);scene.updateMatrixWorld();
 const bits=plantClearanceCases(new GrazerPlants(scene));

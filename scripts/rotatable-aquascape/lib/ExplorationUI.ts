...
export function installExploration(aquarium:Aquarium){
 ...
 const close=()=>{selected=null;panel.hidden=true;main.classList.remove('exploration-open');button.setAttribute('aria-expanded','false');aquarium.identifyMode=false;aquarium.follow(null);aquarium.clearSelection();aquarium.setMagnifier(false);lens=false;};
 ...
 function render(){
  panel.innerHTML=`<div class="explore-heading"><span class="eyebrow">A CLOSER LOOK</span><button id="explore-close" aria-label="Close exploration">×</button></div>
   ...
   <details><summary>How the cardinals behave</summary><p>Watch short browsing stops near planting, individual propulsion and coasting, and returns to companions. Feed fish to see pursuit of sinking flakes.</p><p>Cardinal-tetra field studies inform the foraging. Timing, fin frequency and steering remain illustrative animation choices.</p><a href="./cardinal-behavior-study.md" target="_blank" rel="noopener">Read the study & sources ↗</a></details>`;
  ...
  panel.querySelector<HTMLSelectElement>('#choose-organism')!.onchange=e=>{const [kind,id]=(e.target as HTMLSelectElement).value.split(':');if(kind==='fish')aquarium.identifyFish(+id);if(kind==='plant')aquarium.identifyPlant(id);};
 ...
 setInterval(()=>{if(panel.hidden||selected?.kind!=='fish')return;const text=aquarium.selectedFishStatus,el=panel.querySelector('#observed-behavior');if(el&&el.textContent!==text)el.textContent=text;},400);

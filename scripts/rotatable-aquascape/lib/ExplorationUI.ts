...
export function installExploration(aquarium:Aquarium){
 ...
 function render(){
  panel.innerHTML=`<div class="explore-heading"><span class="eyebrow">A CLOSER LOOK</span><button id="explore-close" aria-label="Close exploration">×</button></div>
   ...
   <label class="learn-field">Choose an organism<select id="choose-organism"><option value="">Tap the tank or choose here</option><optgroup label="Cardinal tetras">${Array.from({length:16},(_,i)=>`<option value="fish:${i}" ${selected?.fishId===i?'selected':''}>Cardinal tetra ${i+1}</option>`).join('')}</optgroup><optgroup label="Shrimp & snails">${Array.from({length:9},(_,i)=>`<option value="animal:${i}" ${selected?.animalId===i?'selected':''}>${i<6?`Cherry shrimp ${i+1}`:`Ramshorn snail ${i-5}`}</option>`).join('')}</optgroup><optgroup label="Plant forms">${Object.entries(plantFacts).map(([id,[name]])=>`<option value="plant:${id}" ${selected?.species===id?'selected':''}>${name}</option>`).join('')}</optgroup></select></label>
   ${selected?`<div class="organism-card"><h2>${selected.name}</h2><p class="organism-name">${selected.subtitle}</p><p>${selected.role}</p><p><strong>Needs</strong> · ${selected.needs}</p><p id="observed-behavior">${selected.behavior}</p>${selected.kind==='invertebrate'?'<button id="inspect-animal" class="learn-action">Close-up · watch its appendages</button>':''}${selected.kind==='fish'?`<button id="follow-fish" class="learn-action">${aquarium.following===selected.fishId?'Stop following':'Follow this tetra'}</button>`:''}</div>`:'<p class="fine">The ring marks your selection. Plant names describe the representative forms used in this aquascape.</p>'}
  ...
  panel.querySelector<HTMLSelectElement>('#choose-organism')!.onchange=e=>{const [kind,id]=(e.target as HTMLSelectElement).value.split(':');if(kind==='fish')aquarium.identifyFish(+id);if(kind==='plant')aquarium.identifyPlant(id);if(kind==='animal')aquarium.identifyAnimal(+id);};

...
float rippleHeight(vec2 p){
 ...
 return broad+cross+fine+sin(inlet*23.-time*3.2)*exp(-inlet*.6)*.0025;
}
`;
...
export class AquariumWater extends T.Group {
 ...
 constructor(){
  ...
  for(const underside of [true,false]){
   const surface=new Reflector(new T.PlaneGeometry(10.08,4.6,160,72),{textureWidth:1024,textureHeight:512,clipBias:.002,multisample:0,shader:{

import {advanceCourtshipSwim} from './TetraCourtship.ts';
...
export type TetraSwim={courtship:number;courtshipAge:number;courtshipDelay:number;mouthReach:number;x:number;y:number;vx:number;vy:number;yaw:number;pitch:number;speed:number;direction:1|-1;sinceTurn:number;elapsed:number;behavior:TetraBehavior;remaining:number;targetX:number;targetY:number;cruiseSpeed:number;effort:number;pectoralEffort:number;seed:number;wasFeeding:boolean;turnRate:number;depthTarget:number;depthRemaining:number;depthBand:number;brain:FishBrain;z:number;vz:number;targetZ:number;depthHeading:number;depthTimer:number;startleRemaining:number;startleCooldown:number;avoidanceRemaining:number;avoidanceZ:number;strokeRemaining:number;powerStroke:boolean;browsing:boolean;pickRemaining:number;pickIn:number;feedingPhase:FeedingPhase;feedingTimer:number;feedingTarget:number|null;feedingBurst:number;};

...
type Species='stem'|'rotala'|'sword'|'anubias'|'carpet';
...
export function buildBotanicalPlants(scene:T.Scene,height:(x:number,z:number)=>number,time:{value:number}){
 ...
 for(const species of ['stem','rotala','sword','anubias','carpet'] as Species[]){
  const p:number[]=[],uv:number[]=[],idx:number[]=[];
  const rows=species==='sword'?20:species==='anubias'?10:6,cols=species==='sword'?6:species==='carpet'?2:4;
 ...
 const colonies=[
  ...
  [3.90,.10,.35,.39,1.57,13,false],[.05,-.20,.38,.28,1.15,9,true]
 ] as const;
 for(const [cx,cz,spreadX,spreadZ,maxH,count,red] of colonies)for(let i=0;i<count;i++){
  ...
  const hue=red?.025+random()*.025:.205+random()*.035,light=.26+random()*.075;
  const grow=(start:number,end:number,offset:T.Vector3)=>{
   ...
   for(let j=1;j<=nodes;j++){
    ...
    const leafCount=red?2:(i%3===0?3:2);
    for(let side=0;side<leafCount;side++){
     const a=nodeAngle+j*1.71+side*Math.PI*2/leafCount+(random()-.5)*.30;
     const maturity=Math.sin(growth*Math.PI*.86),length=(red?.27:.30)*( .55+maturity*.65 )*(.8+random()*.4);
     const direction=V(Math.cos(a),.12+growth*.55+random()*.35,Math.sin(a));
     add(red?'rotala':'stem',at,direction,length,length*(red?.23:.25),hue+(red?(1-t)*.052:0),red?.66:.69,light+(random()-.5)*.055,random()*.9);
    }
   }
  };
  grow(0,1,V(0,0,0));
 ...
 for(const [species,batch] of batches){
  const material=new T.MeshStandardMaterial({color:0xffffff,map:tissue,bumpMap:tissue,bumpScale:species==='sword'?.012:.003,roughness:species==='anubias'?.48:.67,side:T.DoubleSide});

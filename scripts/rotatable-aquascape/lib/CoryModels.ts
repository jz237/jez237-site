...
function geometries(){
 ...
 function ell(p:T.Vector3,s:T.Vector3,color:number,part=9){const g=new T.SphereGeometry(1,20,12);g.scale(s.x,s.y,s.z).translate(p.x,p.y,p.z);return add(g,color,part,p);}
 ...
 const mouth=new T.TorusGeometry(.021,.004,8,24);mouth.rotateX(Math.PI/2);mouth.scale(1,1,.65);mouth.translate(.261,.044,0);add(mouth,0xaa8b64,10,V(.261,.044,0));ell(V(.258,.044,0),V(.016,.003,.011),0x443b30,10);
...
export class CoryModels{
 ...
 constructor(count:number){this.root.name='Dwarf Corydoras group';this.motion=new T.InstancedBufferAttribute(new Float32Array(count*4),4);const gs=geometries();for(let i=0;i<2;i++){const g=gs[i];g.setAttribute('coryMotion',this.motion);const m=new T.MeshStandardMaterial({vertexColors:true,roughness:i?.36:.48,metalness:.08,side:T.DoubleSide,transparent:i===1,opacity:i?.62:1,depthWrite:i===0});const mesh=new T.InstancedMesh(g,m,count);mesh.frustumCulled=false;mesh.castShadow=i===0;mesh.receiveShadow=true;mesh.instanceMatrix.setUsage(T.DynamicDrawUsage);this.meshes.push(mesh);this.root.add(mesh);

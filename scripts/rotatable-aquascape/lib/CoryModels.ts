...
export function coryWave(x:number,phase:number,effort:number){const w=T.MathUtils.clamp((.16-x)/.62,0,1);return Math.sin(phase+x*10)*Math.pow(w,1.45)*(.032+.086*effort);}
/** Rotate each cross-section with the spine so larger bends retain body volume. */
export function coryBend(p:T.Vector3,phase:number,effort:number){const z=coryWave(p.x,phase,effort),angle=Math.atan((coryWave(p.x+.001,phase,effort)-coryWave(p.x-.001,phase,effort))/.002);return new T.Vector3(p.x-Math.sin(angle)*p.z,p.y,z+Math.cos(angle)*p.z);}
function geometries(){
 ...
 for(let i=0;i<p.count;i++){const c=new T.Color(0x96704b);colors.setXYZ(i,c.r,c.g,c.b);}
 // Two raised armor seams follow the body contour rather than floating above it.
 for(const side of [-1,1])for(let i=0;i<19;i++){const x=-.30+i*.025;for(const row of [-.35,.45]){const points=[];for(let j=0;j<6;j++){const a=row+(j/5-.5)*.62,xx=x+.008*Math.sin(j/5*Math.PI),[ry,rz,cy]=coryProfile(xx);points.push(V(xx,cy+Math.sin(a)*ry,side*Math.cos(a)*rz));}tube(points,.0014,0x796044);}}
 for(const side of [-1,1]){ell(V(.20,.151,side*.065),V(.031,.031,.011),0x9a7c50);ell(V(.205,.153,side*.073),V(.024,.024,.011),0x090c0b);ell(V(.213,.163,side*.081),V(.006,.006,.003),0xcde4dc);tube([V(.11,.20,side*.035),V(.13,.135,side*.079),V(.16,.077,side*.056)],.0026,0x806344);}
 const mouth=new T.TorusGeometry(.021,.004,8,24);mouth.rotateX(Math.PI/2);mouth.scale(1,1,.65);mouth.translate(.261,.044,0);add(mouth,0xaa8b64);ell(V(.258,.044,0),V(.016,.003,.011),0x443b30);
 for(const side of [-1,1])for(let k=0;k<3;k++){const root=V(.25-k*.015,.051,side*(.017+k*.009));tube([root,root.clone().add(V(.035,.0,side*.025)),root.clone().add(V(.067-k*.012,-.018,side*(.044+k*.015)))],.0021,0xb89a70,8,root);}
 function fan(root:T.Vector3,edge:T.Vector3[],part:number){const ps:number[]=[],indices:number[]=[];for(let j=0;j<edge.length;j++){ps.push(...root.toArray(),...edge[j].toArray());if(j<edge.length-1)indices.push(j*2,j*2+1,j*2+3);tube([root,root.clone().lerp(edge[j],.45).add(V(0,.001,0)),edge[j]],j===0?.0025:.0011,0x88704d,part,root,true);}
  const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(ps,3));g.setAttribute('uv',new T.Float32BufferAttribute(Array(ps.length/3).fill([0,0]).flat(),2));g.setIndex(indices);g.computeVertexNormals();add(g,0xab956f,part,root,true);
 }
...
export class CoryModels{
 ...
 vec3 coryPose(vec3 p){float phase=coryMotion.x,effort=coryMotion.y;vec3 q=p-coryPivot;float a=0.;if(coryPart>3.5&&coryPart<7.5){a=sin(phase*.61+coryPart*1.8)*(.10+.32*effort);q.yz=mat2(cos(a),sin(a),-sin(a),cos(a))*q.yz;}else if(coryPart>7.5&&coryPart<8.5){a=sin(coryMotion.z*9.+coryPivot.z*27.)*.10;q.xz=mat2(cos(a),sin(a),-sin(a),cos(a))*q.xz;}else if(coryPart>1.5&&coryPart<3.5){q.z+=sin(phase*.47+coryPart)*q.y*.12;}p=coryPivot+q;float spine=corySpine(p.x,phase,effort),bend=atan((corySpine(p.x+.001,phase,effort)-corySpine(p.x-.001,phase,effort))/.002);float lateral=p.z;p.x-=sin(bend)*lateral;p.z=spine+cos(bend)*lateral;return p;}
`+s.vertexShader;s.vertexShader=s.vertexShader.replace('#include <begin_vertex>','coryLocal=position;corySkin=coryPart;vec3 transformed=coryPose(position);');s.fragmentShader=`varying vec3 coryLocal;varying float corySkin;
 ...
 `);s.vertexShader=s.vertexShader.replace('#include <beginnormal_vertex>',`vec3 t=normalize(cross(normal,abs(normal.y)<.9?vec3(0,1,0):vec3(1,0,0)));vec3 b=cross(normal,t);vec3 objectNormal=normalize(cross(coryPose(position+t*.001)-coryPose(position),coryPose(position+b*.001)-coryPose(position)));`);};m.onBeforeCompile=shader;m.customProgramCacheKey=()=>`cory-articulated-v2-${i}`;const depth=new T.MeshDepthMaterial({depthPacking:T.RGBADepthPacking});depth.onBeforeCompile=shader;depth.customProgramCacheKey=()=> 'cory-depth-v2';mesh.customDepthMaterial=depth;}
 }

...
`+s.vertexShader;s.vertexShader=s.vertexShader.replace('#include <begin_vertex>','coryLocal=position;corySkin=coryPart;vec3 transformed=coryPose(position);');s.fragmentShader=`varying vec3 coryLocal;varying float corySkin;
 ...
 `);s.vertexShader=s.vertexShader.replace('#include <beginnormal_vertex>',`vec3 t=normalize(cross(normal,abs(normal.y)<.9?vec3(0,1,0):vec3(1,0,0)));vec3 b=cross(normal,t);vec3 objectNormal=normalize(cross(coryPose(position+t*.001)-coryPose(position),coryPose(position+b*.001)-coryPose(position)));`);};m.onBeforeCompile=shader;m.customProgramCacheKey=()=>`cory-articulated-v3-${i}`;const depth=new T.MeshDepthMaterial({depthPacking:T.RGBADepthPacking});depth.onBeforeCompile=shader;depth.customProgramCacheKey=()=> 'cory-depth-v3';mesh.customDepthMaterial=depth;}
 }

...
vec3 waterTransmission=exp(-vec3(.032,.011,.020)*opticalDistance);
outgoingLight=outgoingLight*waterTransmission+vec3(.016,.032,.025)*waterIllumination*(1.-waterTransmission);
#include <opaque_fragment>`);
   };
   material.customProgramCacheKey=()=>cacheKey+'-bounded-water-depth-v1';material.needsUpdate=true;
  }
 });
}

...
export function advanceShrimpMotion(s:ShrimpMotion,dt:number,flight:number|null,speed:number,turn:number,escapeAge?:number){
 ...
 let curl=posture*(1-s.swimming)+s.swimming*(-.10+.25*launch+.27*landing+.045*Math.sin(s.time*1.35)+.018*Math.sin(s.cycle*.5));
 if(escapeAge!==undefined)curl+=.75*Math.sin(Math.PI*Math.min(1,escapeAge/.38))**2;

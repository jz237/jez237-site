...
export const aquascapeBranches=[
 {points:[[-3.1,.72,.45],[-2.6,1.3,.04],[-2.65,2.1,-.2],[-3.05,2.85,-.32],[-3.25,3.7,-.46]],thickness:1.2,roll:.3},
 {points:[[-2.72,1.35,.15],[-1.95,1.17,.38],[-1.1,1.05,.72],[-.1,.65,1.14],[.63,.49,1.4]],thickness:.8,roll:1.4,reverse:true},
 {points:[[-2.68,1.8,-.22],[-1.86,2.15,-.31],[-1.02,2.72,-.52],[.2,3.3,-.8],[1.4,3.55,-.99]],thickness:.63,roll:.7,reverse:true},
 {points:[[-2.63,2.05,-.28],[-2.15,2.74,-.56],[-1.6,3.65,-.68],[-.8,4.54,-.87]],thickness:.46,roll:2.1},
...
export function bendScannedBranch(sourceGeometry:T.BufferGeometry,branch:typeof aquascapeBranches[number]){
 ...
 const contactRadii=new Array<number>(25).fill(0),geometry=sourceGeometry.clone(),positions=geometry.getAttribute('position') as T.BufferAttribute;
 for(let i=0;i<source.count;i++){
  ...
  const taper=1-.90*Math.pow(t,3.6),y=(source.getY(i)-centerY)*branch.thickness*taper,z=(source.getZ(i)-centerZ)*branch.thickness*taper;

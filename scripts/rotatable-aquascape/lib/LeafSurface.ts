...
export function leafSurfaceMaps(kind:LeafSurfaceKind,seed=2731){
 ...
 for(let y=0;y<1024;y++)for(let x=0;x<512;x++){
  ...
  const grain=(random()-.5),tone=211+18*Math.sin(v*Math.PI)-edge*edge*9+tissue*3+broad*6+grain*7;
  const values=[tone,128+tissue*4+grain*5,218+broad*12+tissue*5+grain*6];
  for(let channel=0;channel<3;channel++){
   const data=pixels[channel].data;data[index]=data[index+1]=data[index+2]=values[channel];data[index+3]=255;
  }
 }
 ...
 const vein=(path:(context:CanvasRenderingContext2D)=>void,width:number,major=false)=>{
  contexts.forEach((context,i)=>{
   context.strokeStyle=i===0?`rgba(247,250,227,${major?.24:.12})`:i===1?`rgba(230,230,230,${major?.72:.38})`:`rgba(154,154,154,${major?.50:.26})`;
 ...
 if(kind==='sword'){
  ...
  for(const x of [82,165,256,347,430])vein(c=>{c.moveTo(x,1010);c.bezierCurveTo(x-8,730,x+9,290,x,8);},x===256?3.4:1.8,true);
  ...
  for(let row=0;row<count;row++)for(const sign of [-1,1]){
   ...
   vein(c=>{c.moveTo(256,base);c.bezierCurveTo(256+sign*65,base-12,256+sign*177,base-reach,256+sign*250,base-reach-10);},kind==='round'?2:1.4,true);
   for(let j=1;j<6;j++){
    const x=256+sign*j*36,y=base-j*reach/6;
    vein(c=>{c.moveTo(x,y);c.quadraticCurveTo(x+sign*20,y-22,x+sign*28,y-46);},.65);
   }
  }
  vein(c=>{c.moveTo(256,1024);c.quadraticCurveTo(252,530,256,0);},3.4,true);
 }

...
import {findCoryForageSite,type CoryForageSite} from './CoryForaging.ts';
...
export type Cory={id:number;position:T.Vector3;previous:T.Vector3;target:T.Vector3;yaw:number;pitch:number;size:number;speed:number;phase:number;effort:number;remaining:number;seed:number;mode:'foraging'|'browsing'|'feeding'|'exploring';blocked:number;time:number;lift:number;visits:Map<string,number>;route:T.Vector3[];biteIn:number;restIn:number;replanIn:number;trail:{point:T.Vector3;yaw:number}[];retreat:number;yieldLeft:number;stalled:number;anchor:T.Vector3;picking?:CoryForageSite};
...
export class Corydoras{
 ...
 private foodPaths=new Map<number,FoodReachability>();
 private feedingOffset=0;
 ...
 constructor(scene:T.Scene,height:(x:number,z:number)=>number,obstacles:Obstacle[]=[],plants?:GrazerPlants,count=6,sessionSeed?:number){this.scene=scene;this.height=height;this.obstacles=obstacles;this.plants=plants;this.routes=new CoryFloorRoutes((x,z)=>this.floor(x,z),(p,f)=>this.solid(p,coryBody(p,f)));this.models=new CoryModels(count);scene.add(this.models.root);
  for(let id=0;id<count;id++){const a:Cory={id,position:V(),previous:V(),target:V(),yaw:id*1.73,pitch:0,size:(.66+id%3*.025)*.8,speed:0,phase:id*2.37,effort:0,remaining:1+id*.7,seed:237+id*7351,mode:id%2?'browsing':'foraging',blocked:0,time:0,lift:0,visits:new Map(),route:[],biteIn:0,restIn:5+id*1.1,replanIn:6+id,trail:[],retreat:0,yieldLeft:0,stalled:0,anchor:V()};
   let found=false;
   for(let k=0;k<(id<6?1600:8000);k++){if(id>=6&&k>=150)a.yaw=random(a)*Math.PI*2;const x=k<150? (id<6?.4:-4)+random(a)*3: -4.55+random(a)*9.1,z=k<150? .5+random(a)*1.6:-2+random(a)*4.1;a.position.set(x,this.floor(x,z),z);if(this.clear(a,a.position,coryForward(a),[])&&(id<6?[-.18,.18]:[-.06,.06]).every(d=>{const q=a.position.clone().addScaledVector(coryForward(a),d);q.y=this.floor(q.x,q.z);return this.clear(a,q,coryForward(a),[]);})){found=true;break;}}
   if(!found)throw new Error(`No unobstructed Corydoras starting position for fish ${id+1}`);a.previous.copy(a.position);a.target.copy(a.position);this.animals.push(a);
 ...
 private choose(a:Cory){
  ...
  if(a.mode==='foraging')this.startPicking(a);
 ...
 feed(onEaten?:()=>void){
  ...
  for(let i=0;i<count;i++){const a=this.animals[(i+this.feedingOffset)%this.animals.length],p=a.position.clone();p.y=5.1;const mesh=new T.Mesh(this.pelletGeometry,this.pelletMaterial);mesh.position.copy(p);this.models.root.add(mesh);this.pellets.push({position:mesh.position,age:0,mesh,onEaten});}this.feedingOffset=(this.feedingOffset+count)%this.animals.length;return count;}
 update(dt:number,waterTime:number,fish:FishContactBody[]=[],grazers:{scale?:number;id?:number;position:T.Vector3;normal:T.Vector3;matrix:T.Matrix4;kind:string}[]=[]){if(dt<=0)return;this.time=waterTime;this.visitorBounds=new WeakMap();this.fishCorrections.clear();const others=grazers.map(a=>grazerBody(a.position,a.normal,V().setFromMatrixColumn(a.matrix,0).normalize(),a.kind==='snail',a.scale??(a.kind==='snail'?.84:.80+(a.id??0)%3*.04)));const visitors=[...others,...fish.map(f=>fishBody(f))];
  ...
  for(let step=0;step<steps;step++)for(const a of this.animals){a.previous.copy(a.position);a.time+=h;a.remaining-=h;a.biteIn=Math.max(0,a.biteIn-h);
   if(this.routes.nodes.length){a.stalled+=a.picking?0:h;if(a.anchor.distanceTo(a.position)>.18){a.anchor.copy(a.position);a.stalled=0;}if(a.stalled>6&&a.trail.length>1&&a.retreat<=0){a.retreat=.65+random(a)*.45;a.stalled=0;a.route=[];const pitch=a.pitch;a.pitch=0;if(!this.clear(a,a.position,coryForward(a),visitors))a.pitch=pitch;}
...
a.replanIn-=h;if(this.routes.nodes.length&&a.replanIn<=0&&!a.picking&&a.mode!=='feeding'&&(!a.route.length||a.position.distanceTo(a.route[0])<.01)){this.choose(a);a.replanIn=5+random(a)*4;}if(Math.floor(a.time*2)!==Math.floor((a.time-h)*2))a.visits.set(`${Math.floor(a.position.x/.65)},${Math.floor(a.position.z/.65)}`,a.time);if(a.route.length&&a.mode!=='feeding'){while(a.route.length&&a.position.distanceTo(a.route[0])<.002)a.route.shift();if(a.route.length)a.target.copy(a.route[0]);else this.choose(a);a.restIn-=h;if(a.mode==='foraging'){if(a.remaining<=0){a.picking=undefined;a.mode='browsing';a.remaining=60;}}else if(a.restIn<=0){if(!this.startPicking(a))a.restIn=1.5;}}else if(a.remaining<=0||a.mode!=='foraging'&&a.position.distanceTo(a.target)<.13)this.choose(a);
   let paths=this.foodPaths.get(a.id);if(!paths){paths=new FoodReachability();this.foodPaths.set(a.id,paths);}
   const food=a.biteIn>0?undefined:this.pellets.filter(p=>p.position.y<this.floor(p.position.x,p.position.z)+.9&&p.position.distanceToSquared(a.position)<4.2**2).sort((p,q)=>p.position.distanceToSquared(a.position)-q.position.distanceToSquared(a.position)).find(p=>paths!.test(p.mesh.id,this.time,a.position,p.position,()=>foodApproach(a.position,p.position,a.yaw,a.pitch,.26*a.size,(q,y,pitch)=>this.solid(q,coryBody(q,V(Math.cos(y),0,-Math.sin(y)),a.size,pitch)),(x,z)=>this.floor(x,z))));
   if(!food&&a.mode==='feeding')this.choose(a);
   if(food&&a.position.distanceTo(food.position)<4.2){a.picking=undefined;a.mode='feeding';a.target.copy(food.position);a.target.y=this.floor(a.target.x,a.target.z);a.remaining=1;a.lift=0;if(coryMouth(a.position,coryForward(a),a.size,a.pitch).distanceTo(food.position)<.085){this.models.bite(a.id);this.removePellet(food,true);a.biteIn=.55+random(a)*.65;a.speed=0;a.mode='foraging';a.remaining=2+random(a)*2;this.startPicking(a);}}
   const steering=a.target.clone().sub(a.position).normalize();for(const b of this.animals){if(b===a||a.route.length&&a.mode!=='feeding')continue;const away=a.position.clone().sub(b.position);away.y=0;const d=away.length();if(d<.85&&d>.001)steering.addScaledVector(away,(.85-d)*3/d);}
   const verticalLeg=a.mode!=='feeding'&&a.route.length>0&&Math.hypot(a.target.x-a.position.x,a.target.z-a.position.z)<.055&&Math.abs(a.target.y-a.position.y)>.000001;
   const desired=a.biteIn>0||a.picking||verticalLeg||steering.lengthSq()<.000001?a.yaw:Math.atan2(-steering.z,steering.x),delta=Math.atan2(Math.sin(desired-a.yaw),Math.cos(desired-a.yaw)),oldYaw=a.yaw,oldForward=coryForward(a);const turnRate=a.mode==='feeding'?3.2:1.35;a.yaw+=clamp(delta,-h*turnRate,h*turnRate);const f=coryForward(a);
   const targetSpeed=a.biteIn>0||a.picking||this.routes.nodes.length&&!a.route.length&&a.mode!=='feeding'||verticalLeg||a.mode!=='feeding'&&a.route.length&&Math.abs(delta)>.002?0:a.mode==='foraging'?.006:a.mode==='feeding'?Math.min(.92+.22*Math.sin(a.time*4.2+a.id*1.9),Math.max(.07,(a.position.distanceTo(a.target)-.18)*2.4)):.16+.16*(.5+.5*Math.sin(a.time*.87+a.id*2.9))**3;
   a.speed+=(targetSpeed*(a.pitch<-.16?.15:1)*(Math.abs(delta)>1?.18:1)-a.speed)*(1-Math.exp(-h*(a.mode==='feeding'?9:4)));const p=a.position.clone();if(a.biteIn>0||a.picking){/* Hold station while searching a food patch. */}else if(a.route.length&&a.mode!=='feeding'){const direction=a.target.clone().sub(a.position),distance=direction.length();if(distance>1e-8&&(verticalLeg||Math.abs(delta)<.002))p.addScaledVector(direction,Math.min(distance,(verticalLeg?.14:a.speed)*h)/distance);}else if(!this.routes.nodes.length||a.mode==='feeding'){p.addScaledVector(f,a.speed*h);const targetY=this.floor(p.x,p.z)+a.lift;p.y=Math.max(this.floor(p.x,p.z),p.y+clamp(targetY-p.y,-h*.20,h*.20));}const floor=this.floor(p.x,p.z);
   ...
   if(a.picking?.leaf)leafContact(a.picking.leaf,a.picking.u!,a.picking.v!,this.time,a.picking.point,V());
   const oldPitch=a.pitch,nibbling=!!a.picking&&a.speed<.025;
   const pitchTarget=nibbling?(a.picking!.kind==='leaf'?clamp(Math.asin(clamp((a.picking!.point.y+.025-a.position.y)/(a.size*.263),-1,1))-.145,-.32,.04)+.018*Math.sin(a.time*7+a.id):nibblePitch(a.time,a.id)):(a.route.length?0:clamp((a.position.y-a.previous.y)/Math.max(.001,a.speed*h),-.12,.12));a.pitch+=(pitchTarget-a.pitch)*(1-Math.exp(-h*3));
   ...
   a.effort+=(clamp(a.speed/(a.mode==='feeding'?.8:.25),0,1)-a.effort)*(1-Math.exp(-h*5));a.phase+=h*(2.5+a.effort*17)*(1+.10*Math.sin(a.time*.9+a.id));this.pose(a);
  }

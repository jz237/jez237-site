...
import {fileColors as colors} from './materials.js?v=9';
...
export class PartsBoard {
 constructor(parts){
  ...
  for(const part of [...parts].sort((a,b)=>order.indexOf(a.group)-order.indexOf(b.group)||a.id-b.id)){
   ...
   ctx.fillStyle=colors[part.group]||'#ccc';ctx.font='600 24px sans-serif';ctx.fillText(`${String(part.id+1).padStart(4,'0')}  ${(part.short||part.group).toUpperCase()}`,9,31);

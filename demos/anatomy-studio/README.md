...
- Finish: X-ray (default), Realistic, Colour-coded, Clay. Shadowing toggles ground-truth ambient occlusion; Zoom detail streams the undecimated tier (`assets/hi/`) for any piece that grows large on screen. Both are on by default on desktop, off on phones. Visible system filter, Labels, Auto orbit, Play sequence.

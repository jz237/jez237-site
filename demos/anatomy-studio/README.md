...
- Finish: X-ray (default), Skin (opaque body: surface only until the body opens; loads the skin file on demand), Realistic, No skin (realistic with the surface regions hidden), Colour-coded, Clay. Shadowing toggles ground-truth ambient occlusion; Zoom detail streams the undecimated tier (`assets/hi/`) for any piece that grows large on screen. Both are on by default on desktop, off on phones. Visible system filter, Labels, Auto orbit, Play sequence.

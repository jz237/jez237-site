...
test('bridges', async (t) => {
  ...
  await t.test('the architecture shot frames the full-fabric zone at structure range', () => {
    const home = presetPatch('architecture');
...
test('structures state plumbing', async (t) => {
  await t.test('the layer and its controls exist with unique keys and sane defaults', () => {
    ...
    assert.equal(LAYERS.structures.def, true, 'the opening diorama includes modeled buildings');
  ...
  await t.test('architectural presets restore their required structures', () => {
    for (const preset of PRESETS) {
      ...
      assert.equal(patch.layers.structures, ['architecture', 'diorama'].includes(preset.id), `${preset.id} layers`);
    }

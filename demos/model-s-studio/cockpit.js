...
async function load(){
 ...
 window.modelSCockpit={getState:()=>({ready,view,software:software.getState(),screenOpen:$('screen-dialog').open,camera:camera.position.toArray(),look:camera.quaternion.toArray(),triangles:renderer.info.render.triangles}),getScreenTargets:()=>screenTargets()};
...
function setCamera(name,immediate=false){
 ...
 resize();$('camera-caption').textContent=p.caption;document.querySelectorAll('[data-camera]').forEach(b=>b.setAttribute('aria-pressed',b.dataset.camera===name));
}
...
function bind(){
 document.querySelectorAll('[data-camera]').forEach(b=>b.onclick=()=>{if(ready)setCamera(b.dataset.camera);});document.querySelectorAll('[data-app]').forEach(b=>b.onclick=()=>openScreen(b.dataset.app));$('open-screen').onclick=()=>openScreen();$('close-screen').onclick=()=>$('screen-dialog').close();$('day-night').onclick=()=>software.change(s=>s.night=!s.night);
 ...
 host.addEventListener('keydown',e=>{if(!ready)return;const keys={ArrowLeft:[-.07,0],ArrowRight:[.07,0],ArrowUp:[0,.05],ArrowDown:[0,-.05]};if(keys[e.key]){e.preventDefault();transition=null;yaw=THREE.MathUtils.clamp(yaw+keys[e.key][0],-1.4,1.1);pitch=THREE.MathUtils.clamp(pitch+keys[e.key][1],-.77,.42);aim();}if(e.key.toLowerCase()==='r')setCamera('driver');});

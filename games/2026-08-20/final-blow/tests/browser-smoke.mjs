...
probe('title-menu', async () => {
    ...
    assert.equal(title.version.text, 'VERSION 5.8.1');
    ...
    assert.equal(title.engineVersion, '5.8.1-ringside');
...
probe('graphic-fatalities', async () => {
    graphicFatalities = await evaluate(client, `(async () => {
      window.__finalBlowQa.quality('battery');
      const reduced = document.querySelector('#reducedMotionToggle');
    ...
    for (const fatality of graphicFatalities) {
      ...
      assert.ok(fatality.peakZoom >= 1.26, `${fatality.fatalityId} must make a restrained cinematic push`);
      ...
      assert.ok(fatality.camera.zoom >= 1.24, `${fatality.fatalityId} aftermath zoom ${fatality.camera.zoom} (${fatality.camera.shot})`);
      assert.equal(fatality.camera.focus, 'fighters', `${fatality.fatalityId} camera must keep the complete exchange framed`);
...
probe('gore-off', async () => {
    ...
    assert.equal(goreOff.snapshot.camera.focus, 'fighters');
    ...
    assert.ok(goreOff.status.peakZoom >= 1.26);
    ...
    assert.ok(reducedFinisher.camera.peakZoom >= 1.26, 'reduced motion keeps a gentle cinematic push');
...
probe('move-viewer', async () => {
 ...
 assert.equal(await evaluate(client,`document.querySelector('#moveViewerFrame').value`),'0.125');
...
probe('crouch-air-recovery', async () => {
  await navigate(client, gameUrl);
  await evaluate(client, `window.__finalBlowQa.commissioner(true)`);
  for (const id of ['jez','benny','alan','ali','commissioner','cyraxx','deathblow','devil','donald','post']) {
    await evaluate(client, `window.__finalBlowQa.fight('${id}','jez')`); await delay(1400);
    for (const airborne of [false,true]) for (const limb of ['punch','kick']) {
      const frames = await evaluate(client, `(() => {
        const qa=window.__finalBlowQa;
        qa.fight('${id}','jez'); qa.positions(400,950); qa.step(.4);
        if (${airborne}) { qa.input(0,{jump:true},1); qa.step(.12); }
        qa.poseTraceReset(); qa.input(0,{light:true,down:${!airborne},limb:'${limb}'},1);
        for(let i=0;i<45;i++){qa.step(1/60);qa.pose();}
        return qa.poseTrace(64,0).filter(p=>p.artBank==='inbetween-approach').map(p=>p.artFrame);
      })()`);
      assert.ok(frames.includes(airborne ? 8 : 12), `${id} ${airborne?'air':'crouch'} ${limb} needs recovery: ${frames}`);
    }
  }
});
...
probe('offline-cache', async () => {
    ...
    assert.equal(offlineCache.entries, 60);
    ...
    assert.equal(controlledReload.version, '5.8.1-ringside');
    ...
    assert.equal(offlineBoot.version, '5.8.1-ringside');
...
probe('mobile-landscape', async () => {
    ...
    assert.equal(landscape.version.text, 'VERSION 5.8.1');
...
probe('touch-finisher', async () => {
    // This probe can run independently of the full mobile menu/dock suite.
    await client.send('Emulation.setTouchEmulationEnabled', {enabled:true,maxTouchPoints:5});
    await client.send('Emulation.setDeviceMetricsOverride', {width:844,height:390,deviceScaleFactor:1,mobile:true});
    await reload(client);
    await evaluate(client, `(()=>{const e=document.querySelector('#reducedMotionToggle');e.checked=true;e.dispatchEvent(new Event('change',{bubbles:true}));})()`);
    ...
    assert.ok(mobileFinisher.camera.peakZoom >= 1.26);
...
probe('painted-only', async () => {
  ...
  assert.equal(await evaluate(client, `window.__finalBlowEngine.version`),'5.8.1-ringside');
});

...
probe('title-menu', async () => {
    ...
    assert.equal(title.version.text, 'VERSION 5.8.3');
    ...
    assert.equal(title.engineVersion, '5.8.3-ringside');
...
probe('offline-cache', async () => {
    ...
    assert.equal(controlledReload.version, '5.8.3-ringside');
    ...
    assert.equal(offlineBoot.version, '5.8.3-ringside');
...
probe('mobile-landscape', async () => {
    ...
    assert.equal(landscape.version.text, 'VERSION 5.8.3');
...
probe('painted-only', async () => {
  ...
  assert.equal(await evaluate(client, `window.__finalBlowEngine.version`),'5.8.3-ringside');
});

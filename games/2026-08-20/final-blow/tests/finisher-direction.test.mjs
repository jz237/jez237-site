...
test('arena scale stays fixed throughout every finisher, including reduced motion',()=>{
 for(const s of Object.values(FINISHER_CHOREOGRAPHY))for(const reduced of [false,true]){
  const end=beats(s)[2].t;
  for(let t=0;t<s.duration;t+=1/120)assert.equal(finisherLens(t,end,reduced),1,'no background scaling');
 }
});

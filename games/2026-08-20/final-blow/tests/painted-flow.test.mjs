...
import { paintedFlowPose } from "../engine/painted-flow.mjs";
...
test("standing strikes reach contact on the real active window and retract during recovery",()=>{
 for(const id of ["jez","benny"]) for(const limb of ["punch","kick"]){
  ...
  const active=limb==="kick"?12:id==="jez"?3:4;
...
test("specials, crouching and airborne attacks retain their own artwork",()=>{
 ...
 for(const patch of [{grounded:false},{crouch:true},{hitstunFrames:5},{def:{id:"post"}},{attacking:{...f.attacking,kind:"special"}}]) assert.equal(paintedFlowPose({...f,...patch}),null);
});

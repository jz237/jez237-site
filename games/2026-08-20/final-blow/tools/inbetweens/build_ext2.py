...
def main():
    ...
    keyed_path = os.path.join(A, f"keyed-{args.fighter}.png")
    keyed.save(keyed_path)

    # Colour transfer onto the fighter's own unified sheet.
    matched_path = os.path.join(A, f"matched-{args.fighter}.png")
    subprocess.run([sys.executable, os.path.join(A, "color_match.py"),
                    f"{G}/assets/unified/{args.fighter}.webp", keyed_path, matched_path], check=True)
    ...
    dest = args.out or f"{G}/assets/unified/{args.fighter}-ext2.webp"
    ...
    with open(os.path.join(A, f"ext2-{args.fighter}.json"), "w") as fh:
        json.dump(side, fh, indent=1)
    pv = Image.new("RGBA", out.size, (40, 40, 40, 255)); pv.alpha_composite(out)
    pv.convert("RGB").resize((640, 640)).save(os.path.join(A, f"preview-{args.fighter}-ext2.png"))

...
export function rockMaterial(){
 ...
 mat.onBeforeCompile=s=>{
  Object.assign(s.uniforms,{cliffColor:{value:landMaps.rock.diff},cliffNormal:{value:landMaps.rock.nor_gl}});
  s.vertexShader=s.vertexShader.replace('#include <common>','#include <common>\nvarying vec3 cliffP;varying vec3 cliffN;').replace('#include <begin_vertex>','#include <begin_vertex>\ncliffP=position;cliffN=normal;');
  s.fragmentShader=s.fragmentShader.replace('#include <common>',`#include <common>\nvarying vec3 cliffP;varying vec3 cliffN;uniform sampler2D cliffColor,cliffNormal;${sampling}`)
   .replace('#include <color_fragment>',`#include <color_fragment>\nvec3 cn=normalize(cliffN),cw=pow(abs(cn),vec3(4.));cw/=dot(cw,vec3(1.));diffuseColor.rgb*=triColor(cliffColor,cliffP*.21,cw);`)
   .replace('#include <normal_fragment_maps>',`#include <normal_fragment_maps>\nvec3 texN=triNormal(cliffNormal,cliffP*.21,cw,cn);normal=normalize(normal+vec3(texN.x,texN.y,0.)*.32);`);
 };mat.customProgramCacheKey=()=> 'coastal-rock-v1';return mat;
}

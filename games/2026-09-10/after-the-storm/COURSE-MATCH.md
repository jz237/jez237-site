...
### Phone controls release — 2026-09-11

Added automatically enabled touchscreen controls with a manual Phone controls menu checkbox. Independent pointer ownership supports steering and throttle together, brake, wave absorption, rescue, camera switching, flips/rolls, dive, slide and bow trim. Pointer cancellation, capture loss, pause, visibility loss and resize clear held controls. Touch drives player 1 using the existing input/physics mapping; player 2 still needs a keyboard or gamepad.

Portrait 390x844 and landscape 844x390 browser previews were visually inspected, including the expanded stunt tray and pause/resume. Six focused touch/keyboard/gamepad input tests pass; catalog syntax and protected site/service-worker checks pass. No browser errors appeared. Physical-phone multitouch and GPU performance were not measured. Existing five course/stunt regression failures and original-course parity limitations remain; this input-only update does not claim to resolve them.

### Smart demo tour — 2026-09-11

Added a user-started smart demo with eleven scenes: one real lap on all nine venues plus the guided Marine Fortress and Port Blue stunt routes. The driver uses existing helm/throttle/brake and authored stunt guidance; progress is earned by the normal physics and scoring. A progress watchdog requests the ordinary rescue control after a ten-second stall. Scenes advance after a completed lap/run or a 115-second safety limit, with a four-second transition interval. Pause freezes the tour; Next coast skips a scene; Exit returns to the menu. A gentle camera orbit follows the hull and stops overriding the camera after a manual drag.
...
### Title launch, faster demo pack and physical wake crossings — 2026-09-11

Moved Watch smart demo directly beneath the main title, before the descriptive text and race settings. Demo races now start every rider at full engine power and use a speed controller with turn/obstacle braking. All nine demo race laps complete; measured peak speeds were 76–86 km/h, with typical whole-lap averages 44–58 km/h. Stunt approach guidance retains its careful pace. Ranked race AI pace and rider top-speed tuning are unchanged.

Persistent wake packets now broaden and increase from 0.12 to 0.32 height gain as emitted power rises from 1.05 to 1.4. This power already depends on source speed and immersed hull fraction. The CPU surface and GPU displacement/normal derivatives share the frequency, envelope and decay equations. Every rider samples the resulting surface with the existing twelve-patch buoyancy/pressure model, so a wake crossing changes pitch, roll, load, lift and landing; no jump impulse or position/progress injection was added. Spray emission, chine spread and ballistic sheets are more visible, with a lower mobile-quality particle cap.

An initial unconditional wake increase regressed several slow passages and was replaced by the power-dependent blend. The subsequent full regression was 250/256: the five previously known course/stunt failures plus a new two-ski fixture whose source had not yet reached racing speed. Corrected that fixture to allow 60 metres of ordinary acceleration before the crossing, retained a separate gentle-launch-wake no-jump case, and strengthened the fast-crossing airtime/height assertions. Final focused release checks 19/19 pass, including all demo scenes, speed calibration and three wake-crossing tests. Production physics was unchanged after that full run; only the fixture/test coverage changed. No aggregate full-suite rerun with the additional test is claimed.

Isolated wake train: 3 m/s crossing stays in contact; 26 m/s crossing reaches about 0.68 m hull clearance. Two actual moving skis with ordinary throttle controls: the racing-speed wake crossing reaches about 0.71 m and 0.23 s airborne, while the same crossing without the leading ski remains on the water. These are deterministic fixtures, not a fluid-dynamics calibration. Phone title and landscape racing visuals were inspected without browser errors. Original-course fidelity and the five existing specialized failures remain open.

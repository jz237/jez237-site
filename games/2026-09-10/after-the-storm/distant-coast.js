...
export function makeDistantCoast(root,course){
 ...
 for(let layer=0;layer<3;layer++){
  ...
  for(let island=0;island<3;island++){
   const a=offset+island*2.09,cx=Math.sin(a)*radius,cz=Math.cos(a)*radius,width=150+random()*110,depth=95+random()*65,height=island===1?75+random()*35:105+random()*60,rockSeed=random()*1000;
   ...
   if(layer<2&&!cold&&!urban)for(let j=0;j<(layer===0?480:150);j++){
    ...
    if(p[1]<5||slope(u,v)>1.35||geologyNoise(u*5,v*5,rockSeed)<.37)continue;
   ...
   if(layer===0)for(let j=0;j<65;j++){
    ...
    boulders.push({p,scale:4+random()*10,angle:random()*6.28});
   }
  }

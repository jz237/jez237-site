...
uniform vec3 eye,sun;uniform float near,far;uniform vec4 wake[48];uniform float wakeHeading[48];uniform vec3 reefs[7];
...
float floorH(vec2 p){if(customTerrain>.5){vec2 encodedHeight=texture2D(terrainMap,clamp(p/terrainSpan+.5,vec2(0.),vec2(1.))).rg;return -16.+dot(encodedHeight,vec2(256.,1.))/257.*100.;}float x=p.x,z=p.y;float outer=length(vec2(x/1.04,(z+15.)/1.25));float g=-7.+max(0.,outer-125.)*.22;g+=sin(x*.04)*.5+sin(z*.053+x*.025)*.65;g+=5.8*exp(-((x+77.)*(x+77.)+(z-25.)*(z-25.))/1500.);g+=5.4*exp(-((x-87.)*(x-87.)+(z+52.)*(z+52.))/1350.);g+=2.4*exp(-((x+32.)*(x+32.)+(z+166.)*(z+166.))/1600.);if(z< -170.)g-=min(16.,(-z-170.)*.16)*exp(-x*x/12500.);return g;}
void main(){
 vec2 p=worldP.xz;vec3 V=normalize(eye-worldP);float dist=length(eye-worldP);
 vec3 surface=waveSurface(p);float e=.10;
 vec2 wakeSlope=vec2(jetWake(p+vec2(e,0))+impactHeight(p+vec2(e,0))-jetWake(p-vec2(e,0))-impactHeight(p-vec2(e,0)),jetWake(p+vec2(0,e))+impactHeight(p+vec2(0,e))-jetWake(p-vec2(0,e))-impactHeight(p-vec2(0,e)))/(2.*e);
 ...
 for(int i=0;i<48;i++){vec4 w=wake[i];float age=time-w.z;if(age>=0.&&age<14.&&w.w>.02){vec2 delta=p-w.xy-vec2(.035,-.024)*age;float spread=.37+age*.40;if(dot(delta,delta)<(spread+3.)*(spread+3.)){

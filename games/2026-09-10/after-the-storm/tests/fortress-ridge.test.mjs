...
test('a timed wave carries the hull across the northern Fortress ridge without impacts or phantom buoy misses',()=>{
 for(const ridgeWait of [4.25,4.5,4.75]){const c=getCourse('citadel'),s=createRace({course:c}),r=s.racers[0];s.verifyFortressRidge=true;s.ridgeWait=ridgeWait;let ridgeFrames=0,hits=0,clearance=Infinity;
 for(let i=0;i<36000&&s.phase!=='results';i++){
  ...
  if(ridgeWait!==4.5&&r.fortressRidge?.done)break;
 }
 ...
 if(ridgeWait===4.5){assert.equal(s.phase,'results');assert.equal(r.passed,c.gates.length*3);}
 }
});

...
import {createRace,stepRace,aiInput,adjudicateGate,settings,COVE,gateCoordinates,outsideCourse} from '../race-core.js';
...
test('all four riders complete a three-lap race using ordinary helm input',()=>{
 for(let rider=0;rider<4;rider++){
  ...
  for(let i=0;i<36000&&s.phase!=='results';i++)stepRace(s,aiInput(s,s.racers[0]),1/60);
  const p=s.racers[0];assert.equal(s.phase,'results');assert.equal(p.dq,'');assert.equal(p.lapTimes.length,3);assert.equal(p.passed,COVE.gates.length*3);assert.equal(p.misses,0);assert.ok(p.finishTime>60&&p.finishTime<600);assert.ok(s.racers.every(r=>r.passed>30));
 }
});
test('red/right and yellow/left use the riders forward view, not world X',()=>{
 for(const side of [-1,1]){
  const s=createRace(),r=s.racers[0];r.next=s.course.gates.findIndex(g=>g.side===side);const g=s.course.gates[r.next];
...
test('course out is cumulative only while outside, and pause stops time',()=>{
 ...
 const s=createRace({mode:'time',course:{...COVE,ground:()=>-10,rocks:[]}});s.phase='running';const r=s.racers[0];r.x=-200;r.z=0;assert.ok(outsideCourse(s.course,r.x,r.z));
 ...
 s.phase='paused';const t=s.time;stepRace(s,{throttle:1},1/60);assert.equal(s.time,t);s.phase='running';r.x=-200;r.z=0;assert.ok(outsideCourse(s.course,r.x,r.z));for(let i=0;i<305;i++)stepRace(s,{},1/60);assert.equal(s.phase,'results');assert.equal(s.result.dq,'Outside course for five seconds');
});

...
test('nine distinct venues and all four classes complete with ordinary helm input',()=>{const shapes=new Set();for(const base of COURSES){shapes.add(JSON.stringify(base.anchors));for(let difficulty=0;difficulty<4;difficulty++){const course=getCourse(base.id,difficulty),s=createRace({course,difficulty});for(let i=0;i<36000&&s.phase!=='results';i++)stepRace(s,aiInput(s,s.racers[0]),1/60);assert.equal(s.phase,'results',base.id+'/'+difficulty);assert.equal(s.result.dq,'',base.id+'/'+difficulty);assert.equal(s.racers[0].misses,0,base.id+'/'+difficulty);assert.equal(s.racers[0].passed,course.gates.length*3);}}assert.equal(shapes.size,9);});
test('reverse reverses traversal and difficult classes add obstacles',()=>{const n=getCourse('amber'),h=getCourse('amber',2),r=getCourse('amber',3);assert.ok(h.gates.length>n.gates.length);assert.equal(h.rocks.length,10);assert.equal(n.rocks.length,0);assert.ok(h.gates[0].tx*r.gates[0].tx+h.gates[0].tz*r.gates[0].tz<-.99);for(const c of [n,h,r])for(const g of c.gates)assert.ok(c.ground(g.x,g.z)<-2);});

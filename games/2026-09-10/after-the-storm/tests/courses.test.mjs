...
test('reverse reverses traversal and difficult classes add obstacles',()=>{const n=getCourse('amber'),h=getCourse('amber',2),r=getCourse('amber',3);assert.ok(h.gates.length>n.gates.length);assert.equal(h.rocks.length,0);assert.equal(n.rocks.length,0);assert.ok(h.gates[0].tx*r.gates[0].tx+h.gates[0].tz*r.gates[0].tz<-.99);for(const c of [n,h,r])for(const g of c.gates)assert.ok(c.ground(g.x,g.z)<-2);});

...
export function portStuntLayout(){
 ...
 const ring=i=>({...rings[i],kind:'ring',speed:i>=3&&i<=10?9:13}),cp=i=>({...checkpoints[i],kind:'checkpoint'}),ramp=i=>({...ramps[i],kind:'ramp',speed:12}),way=row=>{const p=map(row);return {...p,...tangent(p),kind:'waypoint',speed:row[0]===263&&row[1]===279?5:9};};

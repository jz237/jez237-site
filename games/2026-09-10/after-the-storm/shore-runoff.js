...
export function createRunoff(n,cell,bed){return {n,cell,bed:Float32Array.from(bed),water:new Float32Array(n*n),fx:new Float32Array(n*n),fz:new Float32Array(n*n)};}
export function stepRunoff(g,dt,{sea=null,infiltration=.0015}={}){
 ...
 const delta=new Float32Array(water.length),k=Math.min(.2,dt*1.5/cell);
 for(let z=0;z<n;z++)for(let x=0;x<n;x++){const i=z*n+x;for(const [j,axis] of [[x+1<n?i+1:-1,0],[z+1<n?i+n:-1,1]]){if(j<0)continue;

...
export function sprayLaunch(r,kind,side,random=Math.random){
 ...
 const out=impact?(plume.lateral*(.4+random()*.6)):(.8+power*5.8+Math.max(0,slip*side)*.65+catchSpray*3)*contact;
 ...
 return {x:r.x+fx*aft+rx*(jet?0:side*.61),y:h.y-(h.pitch||0)*aft+(h.roll||0)*(jet?0:side*.61)+(jet?-.10:.035),z:r.z+fz*aft+rz*(jet?0:side*.61),
  ...
  vy:(jet?1.6+power*2.8:impact?.7+random()*h.impact*.55*plume.rise:.7+power*2.0+Math.max(0,h.waterVelocity)*.12)+Math.max(0,h.vy-(h.pitchVelocity||0)*aft+(h.rollVelocity||0)*(jet?0:side*.61))*.25,

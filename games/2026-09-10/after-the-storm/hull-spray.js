...
export function sprayLaunch(r,kind,side,random=Math.random){
 ...
 const out=impact?(1+random()*h.impact*.7):(.5+power*3.8+Math.max(0,slip*side)*.55+catchSpray*3)*contact;
 const back=jet?5+Math.sqrt(Math.max(0,r.throttle))*15+(h.reengagement||0)*6:impact?1:1.8;
 ...
 return {x:r.x+fx*aft+rx*(jet?0:side*.61),y:h.y-(h.pitch||0)*aft+(h.roll||0)*(jet?0:side*.61)+(jet?-.10:.035),z:r.z+fz*aft+rz*(jet?0:side*.61),
  ...
  vy:(jet?1.3+power*1.9:impact?.7+random()*h.impact*.55:.45+power*1.3+Math.max(0,h.waterVelocity)*.12)+Math.max(0,h.vy-(h.pitchVelocity||0)*aft+(h.rollVelocity||0)*(jet?0:side*.61))*.25,
  vz:r.vz*carry-fz*back+rz*side*(jet?random()*.45:out),
  size:mist?.08+random()*.13:jet?.025+random()*.045:impact?.035+random()*.09:.02+random()*.07,life:mist?1.5+random()*1.4:.45+random()*.6,mist};
}

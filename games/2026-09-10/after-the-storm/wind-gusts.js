...
export function gustAt(x,z,t,storm=0){
 ...
 const strength=clamp(band*variation),speed=2+storm*10+strength*(5+storm*6);
 return {x:speed*.86,z:-speed*.51,strength,speed};
}
export function gustHeight(x,z,t,storm=0){const g=gustAt(x,z,t,storm);return g.strength*(.028+storm*.055)*Math.sin(x*.8+z*.45-t*4.5)*Math.cos(x*.23-z*.51-t*1.8);}
export const gustGLSL=`
vec3 gustAt(vec2 p,float t,float s){
 ...
 float speed=2.+s*10.+strength*(5.+s*6.);return vec3(speed*.86,-speed*.51,strength);
}
float gustHeightAt(vec2 p,float t,float s){return gustAt(p,t,s).z*(.028+s*.055)*sin(p.x*.8+p.y*.45-t*4.5)*cos(p.x*.23-p.y*.51-t*1.8);}

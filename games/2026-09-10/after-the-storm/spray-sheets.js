...
export function makeSpraySheets(scene){const segments=10,positions=new Float32Array((segments+1)*2*3*2),uvs=new Float32Array((segments+1)*2*2*2),indices=[];
 ...
 return {mesh,update(r,time,storm){material.uniforms.time.value=time;material.uniforms.storm.value=storm;const power=Math.min(1,r.speed/20);material.uniforms.opacity.value=power*.68;mesh.visible=r.hydro.wet>.05&&r.speed>3;
  for(let side=0;side<2;side++){const sign=side?1:-1,q=sprayLaunch(r,'chine',sign,()=>.5),contact=sign>0?r.hydro.starboardWet:r.hydro.portWet,rx=Math.cos(r.heading),rz=-Math.sin(r.heading);
   for(let j=0;j<=segments;j++){const age=j/segments*.42,width=(.035+age*.9)*contact;for(let k=0;k<2;k++){const spread=(k-.5)*width,index=(side*(segments+1)*2+j*2+k)*3;positions.set([q.x+(q.vx-r.vx)*age+rx*spread,q.y+q.vy*age-4.905*age*age,q.z+(q.vz-r.vz)*age+rz*spread],index);}}

import {spatialEngine} from './immersion-model.js';
...
export class SampleSound{
 ...
 async load(loader=url=>fetch(url).then(r=>{if(!r.ok)throw Error('Sound asset '+r.status);return r.arrayBuffer();})){
  ...
  for(let player=2;player<5;player++)this.loop('fast',player);this.loop('wind',-1);if(this.buffers.music)this.loop('music',-1);return true;
 }
 ...
 update(speed,storm,wet,companion,throttle=0,music=false,mode='stereo',engineMuted=false,paused=false){
  ...
  for(const l of this.loops){if(l.player>=2)continue;const second=l.player===1,p=second?companion:{speed,wet,throttle},m=engineMix(p?.speed||0,p?.wet??1,p?.throttle??throttle);let gain=0,rate=1,cutoff=16000,pan=0;
 ...
 updateRivals(listener,racers,paused=false,camera=null){if(!this.ready)return;const t=this.ctx.currentTime,near=racers.filter(r=>r!==listener&&r.mount!=='dolphin').sort((a,b)=>Math.hypot(a.x-listener.x,a.z-listener.z)-Math.hypot(b.x-listener.x,b.z-listener.z)).slice(0,3);
  this.rivalMix=[];for(const l of this.loops.filter(l=>l.player>=2)){const r=near[l.player-2];if(!r||paused){l.gain.gain.setTargetAtTime(0,t,.08);continue;}const spatial=spatialEngine(camera?{...listener,...camera}:listener,r),m=engineMix(r.speed*.45,r.hydro?.intake??1,r.throttle||0);l.gain.gain.setTargetAtTime(m.fast*spatial.gain*.65,t,.10);l.source.playbackRate.setTargetAtTime(m.fastRate*spatial.doppler,t,.08);l.pan.pan.setTargetAtTime(spatial.pan*stereoWidth(this.mode),t,.06);l.filter.frequency.setTargetAtTime(spatial.cutoff,t,.12);this.rivalMix.push({...spatial,id:r.id});}
 }

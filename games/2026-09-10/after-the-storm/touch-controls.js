...
export function mountTouchControls({onCamera}){
 ...
 root.innerHTML=`<div class="touch-tools"><button type="button" data-key="KeyR">Rescue</button><button type="button" id="touchCamera">Camera</button><button type="button" id="touchTricks" aria-expanded="false">Stunts</button></div>
 ...
 <div class="touch-driving"><div class="touch-steering"><button data-key="KeyA" aria-label="Steer left">◀</button><button data-key="KeyD" aria-label="Steer right">▶</button><small>HOLD TO STEER</small></div><div class="touch-pedals"><button data-key="KeyB">Absorb</button><button data-key="Space">Brake</button><button class="touch-gas" data-key="KeyW">GO</button></div></div>`;
 document.body.append(root);
 const choice=document.createElement('label');choice.className='touch-choice';
 choice.innerHTML='<input type="checkbox" id="touchEnabled"> Phone controls <small>Hold GO and a steering arrow together. Drag open water to orbit. Landscape recommended. Touch drives player 1.</small>';
 document.querySelector('#start').before(choice);
 const toggle=choice.querySelector('input');toggle.checked=navigator.maxTouchPoints>0||matchMedia('(pointer: coarse)').matches;
 let active=false;
 function reset(){held.clear();root.querySelectorAll('[data-key]').forEach(b=>{b.classList.remove('held');b.setAttribute('aria-pressed','false');});}
 ...
 return {keys:()=>held.keys(),reset,setActive(value){if(active!==value){active=value;update();}}};
}

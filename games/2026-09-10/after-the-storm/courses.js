...
export function mappedGates(route,buoys){
 ...
 return {x:b.x-tz*b.side*offset,z:b.z+tx*b.side*offset,tx,tz,side:b.side,offset,bx:b.x,bz:b.z,width:b.width??23,routeIndex:best,approach:b.approach};})];
}
...
export function getCourse(id='greyhaven',difficulty=0){const base=COURSES.find(c=>c.id===id)||COURSES[0],reverse=difficulty===3,level=Math.min(2,difficulty),anchors=base.anchorsByClass?.[difficulty]||(level===2&&base.expertAnchors?base.expertAnchors:base.anchors);let points=sampleRoute(anchors,24+level*4);if(reverse)points=[points[0],...points.slice(1).reverse()];
 ...
 if(base.finishLine){const g=course.gates[0],[a,b]=base.finishLine,dx=b[0]-a[0],dz=b[1]-a[1],length=Math.hypot(dx,dz),direction=reverse?-1:1;g.tx=dz/length*direction;g.tz=-dx/length*direction;g.z=a[1];const sides=[a,b].map(p=>-(p[0]-g.x)*g.tz+(p[1]-g.z)*g.tx);g.spanMin=Math.min(...sides);g.spanMax=Math.max(...sides);course.finishBypass=reverse?0:base.finishBypass;}

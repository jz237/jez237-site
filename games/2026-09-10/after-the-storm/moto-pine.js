...
export const motoPine=await (async()=>{try{
 const url=name=>assetURL('assets/scenery/'+name);
 const [metaResponse,bin,bark,bough]=await Promise.all([fetchAsset(url('pine.json')),loadBinary('assets/scenery/pine.bin'),loadTexture('assets/scenery/pine-bark.jpg'),loadTexture('assets/scenery/pine-bough.webp')]);if(!metaResponse.ok)throw Error('Pine asset unavailable');

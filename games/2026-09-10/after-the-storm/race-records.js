...
export function addScore(save,entry){if(!Number.isFinite(entry.score)||entry.score<0)throw new Error('Invalid stunt score');save.scores??={};const key=entry.course+'/'+(entry.difficulty||0),row={initials:cleanInitials(entry.initials),score:Math.floor(entry.score),rider:entry.rider};const rows=[...(save.scores[key]||[]),row].sort((a,b)=>b.score-a.score).slice(0,3);save.scores[key]=rows;return {rank:rows.indexOf(row)+1};}
...
function recordParts(key,count){const p=key.split('/');if(p.length!==count||!courseIds.has(p[0])||!['0','1','2','3'].includes(p[1])||(count===3&&!['3','4','5','6','9'].includes(p[2])))fail('Unknown course record.');return p;}
...
export function eraseRecords(save,{course,difficulty=0,laps=3,kind='time'}){if(!courseIds.has(course))fail('Unknown course.');const bucket=kind==='score'?save.scores:save.records,key=kind==='score'?course+'/'+difficulty:recordKey({course,difficulty,laps});const previous=bucket[key];delete bucket[key];return {kind,key,previous:previous?structuredClone(previous):null};}

...
export function makeCourseWorld(scene,course,ocean,{freeRide=false}={}){
 ...
 const materials=[],geometries=[],textures=[],buoys=[],animated=[],lights=[];
 ...
 const palette=cold?{sand:0x94bdc9,rock:0xadc8d5,grass:0xe5f0f0}:theme==='port'||night?{sand:0x737e81,rock:0x899091,grass:0x828b83}:theme==='resort'||theme==='island'?{sand:0xd8c99c,rock:0xa7a58a,grass:0x759358}:{};
 ...
 if(!freeRide){
 const finish=course.gates[0],gantry=new T.Group();gantry.position.set(finish.x,0,finish.z);gantry.rotation.y=Math.atan2(finish.tx,finish.tz);root.add(gantry);for(const x of[-21,21]){cylinder(x,3.4,0,.16,7,steel,gantry);box(x,.15,0,2,.5,3,yellow,gantry);}box(0,6.7,0,42,1.6,.4,black,gantry);label(course.name.toUpperCase()+' / FINISH',0,6.8,.25,34,gantry);label(course.name.toUpperCase()+' / FINISH',0,6.8,-.25,34,gantry).rotation.y=Math.PI;
 }
 ...
 // Ramp deck geometry matches the incline used by the hull contact solver.
 for(const ramp of course.ramps||[]){const g=new T.Group();g.position.set(ramp.x,0,ramp.z);g.rotation.y=Math.atan2(ramp.tx,ramp.tz);root.add(g);const deck=box(0,ramp.height/2+.08,0,ramp.width,.20,Math.hypot(ramp.length,ramp.height),yellow,g);deck.rotation.x=-Math.atan2(ramp.height,ramp.length);for(const side of [-1,1]){rod([side*ramp.width*.48,.12,-ramp.length/2],[side*ramp.width*.48,ramp.height+.12,ramp.length/2],.07,white,g);cylinder(side*ramp.width*.42,ramp.height/2,ramp.length*.4,.10,ramp.height,steel,g);}label('JUMP',0,ramp.height+.35,ramp.length/2+.04,ramp.width*.8,g);}
 const ringMeshes=[];for(const [i,r] of (course.rings||[]).entries()){const ring=add(new T.TorusGeometry(r.radius,.095,8,40),r.type==='dive'?iceMat:yellow,r.x,r.y,r.z);ring.rotation.y=Math.atan2(r.tx,r.tz);ring.userData.dynamic=true;ringMeshes.push({ring,index:i});}
 for(const cp of course.checkpoints||[]){const g=new T.Group();g.position.set(cp.x,0,cp.z);g.rotation.y=Math.atan2(cp.tx,cp.tz);root.add(g);for(const side of [-1,1]){cylinder(side*cp.width,3.2,0,.15,6.4,steel,g);box(side*cp.width,.1,0,2,.5,3,white,g);}label('CHECKPOINT '+(cp.section+1),0,6.3,0,30,g);label('CHECKPOINT '+(cp.section+1),0,6.3,-.05,30,g).rotation.y=Math.PI;}
 ...
 if(theme==='coast'||theme==='resort'||theme==='island'||theme==='park'){
 ...
 if(night){
  ...
  const p=course.gates[5],bridge=new T.Group();bridge.position.set(p.x,8,p.z);bridge.rotation.y=Math.atan2(p.tx,p.tz);root.add(bridge);box(0,0,0,100,1,9,steel,bridge);for(const x of[-40,40])box(x,-4,0,3,8,10,stone,bridge);for(let x=-45;x<46;x+=5)box(x,1.2,-4,.18,2,.18,cityLight,bridge);label('NEON REACH',0,2,4.6,33,bridge,'#9ceaff');
 }
 ...
 return {root,update(time,storm,stunt=null,openedAt=Infinity){scenery.update(time,storm,ocean.quality);if(gateMesh){const opening=passageOpening(passage,time,openedAt);gateMesh.position.y=-1+opening*(passage.clearance+1.5);gateSignal.color.setHex(opening>.98?0x66b88b:0xc65a32);gateSignal.emissive.setHex(opening>.98?0x2d8052:0x8e3018);}if(stunt)for(const item of ringMeshes)item.ring.visible=stunt.ringStatus[item.index]!=='hit';const surface=(x,z,t)=>wave(x,z,t,storm);for(const b of buoys){const pose=floatingPose(b.x,b.z,time,surface);b.g.position.y=pose.y;b.g.rotation.x=pose.pitch;b.g.rotation.z=pose.roll;}for(const a of animated){const p=obstaclePosition(a.obstacle,time),heading=time*.02+a.obstacle.x,pose=floatingPose(p.x,p.z,time,surface,a.obstacle.r,heading);a.o.position.set(p.x,pose.y+a.offset,p.z);a.o.rotation.set(pose.pitch,heading,pose.roll,'YXZ');}ocean.mat.uniforms.reefs.value.forEach((v,i)=>{const q=course.rocks[i];if(q){const p=obstaclePosition(q,time);v.set(p.x,p.z,q.r);}});},dispose(){scenery.dispose();scene.remove(root);for(const g of new Set(geometries))g.dispose();for(const m of new Set(materials))m.dispose();for(const t of textures)t.dispose();}};
}

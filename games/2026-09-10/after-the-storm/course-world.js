...
export function makeCourseWorld(scene,course,ocean,{freeRide=false}={}){
 ...
 for(const obstacle of course.rocks){if(obstacle.type==='ice'){const o=rock(obstacle.x,obstacle.z,obstacle.r,true);animated.push({o,obstacle,offset:.3});}else if(obstacle.type==='post'){cylinder(obstacle.x,.65,obstacle.z,obstacle.r,3.8,wood);cylinder(obstacle.x,1.3,obstacle.z,obstacle.r+.04,.2,white);}else if(obstacle.type==='ball'){const o=add(new T.SphereGeometry(obstacle.r,16,12),steel,obstacle.x,.1,obstacle.z);o.userData.dynamic=true;animated.push({o,obstacle,offset:.1});}else if(obstacle.type==='timber'){const g=new T.Group();g.userData.dynamic=true;root.add(g);box(0,0,0,1.6,.22,.3,wood,g);box(-.35,.12,0,.12,.06,.32,wood,g);animated.push({o:g,obstacle,offset:.06});}else if(obstacle.type==='crate'){const g=new T.Group();g.userData.dynamic=true;root.add(g);box(0,0,0,1.5,1.5,1.5,wood,g);for(const side of [-1,1]){box(side*.64,0,0,.12,1.57,1.57,steel,g);box(0,0,side*.64,1.57,1.57,.12,steel,g);}animated.push({o:g,obstacle,offset:.38});}else buoy(obstacle.x,obstacle.z,yellow,1.0);}

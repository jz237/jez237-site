...
vec3 waveSurface(vec2 p){vec2 q=p;float h;vec2 d,g;mat2 J;for(int j=0;j<4;j++){q=p-swellShift(q);}swellAt(q,h,d,g,J);float det=max(.12,J[0][0]*J[1][1]-J[1][0]*J[0][1]);vec2 slope=vec2(J[1][1]*g.x-J[0][1]*g.y,-J[1][0]*g.x+J[0][0]*g.y)/det;return coastalSurface(p,vec3(h,slope)+surfSurface(p))+waveTrainSurface(p);}`;

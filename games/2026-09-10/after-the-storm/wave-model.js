...
export function sampleSwell(x,z,t,storm=0){let qx=x,qz=z;const scale=1+storm*2.3;
 ...
 let y=0;for(const [dx,dz,k,a,w,ph] of WAVES){const f=(qx*dx+qz*dz)*k-t*w+ph;y+=(Math.sin(f)+.12*Math.sin(2*f))*a*scale;}return y;
}
...
vec3 displacedSurface(vec2 q){float h;vec2 d,g;mat2 J;swellAt(q,h,d,g,J);return vec3(q.x+d.x,h,q.y+d.y);}
vec3 waveSurface(vec2 p){vec2 q=p;float h;vec2 d,g;mat2 J;for(int j=0;j<4;j++){q=p-swellShift(q);}swellAt(q,h,d,g,J);float det=max(.12,J[0][0]*J[1][1]-J[1][0]*J[0][1]);vec2 slope=vec2(J[1][1]*g.x-J[0][1]*g.y,-J[1][0]*g.x+J[0][0]*g.y)/det;return vec3(h,slope);}`;

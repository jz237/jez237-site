(() => {
  ...
  function readHighScore() {
    try {
      return Number(localStorage.getItem("arkanoid-recoded-high") || 0);
    } catch (_) {
      return 0;
    }
  }
  ...
  function saveHighScore() {
    ...
    highScore = score;
    try {
      localStorage.setItem("arkanoid-recoded-high", String(highScore));
    } catch (_) {
  ...
  function stopMusic() {
    ...
    if (audioContext && musicBus) {
      ...
      musicBus.gain.setTargetAtTime(.0001, audioContext.currentTime, .035);
    }
  }
  ...
  function drawOverlay() {
    ...
    if (state === STATES.TITLE) {
      shade();
      logo("ARKANOID", "RECODED");
      centered("A CLEAN-ROOM 68000-ERA REMAKE", 374, 17, "#7bb5d9");
      centered("ORIGINAL PAULA SFX · NEW TRACKER SCORE", 410, 15, "#ff8db0");

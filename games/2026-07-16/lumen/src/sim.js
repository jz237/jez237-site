...
import { ECON, TOWERS, ENEMIES, MAPS, waveComp, depthScale, WORLD_W, WORLD_H, HORIZON_Y, COLORS } from './content.js';
...
export class Sim {
  ...
  towerStats(t) {
    ...
    return { damage: lv.damage, range: lv.range, rate: lv.rate, chain: lv.chain || 0 };
  }
  ...
  stepTowers(dt) {
    for (const t of this.towers) {
      ...
      for (const e of this.enemies) {
        ...
        if (dx * dx + dy * dy <= r2 && e.s > bestS && e.hp > 0) { best = e; bestS = e.s; }
      }
      if (!best) continue;
      t.cool = 1 / st.rate;
      t.charge = 0;
      if (t.type === 'coral') {
        ...
        this.projectiles.push({
          ...
          splash: t.def.splash, color: t.def.color, from: t.id,
          vx: 0, vy: 0, t: 0,
        });
        this.emit('fire', { tower: t.id, type: 'coral', x: t.x, y: t.y });
        ...
        for (let c = 0; c < st.chain && cur; c++) {
          ...
          cur.slow = Math.max(cur.slow, 0.4); // static charge briefly slows
          seen.add(cur.id);
          src = { x: cur.x, y: cur.y };
          dmg *= t.def.chainFall;
        ...
        this.arcs.push({ hits, t: 0, dur: 0.22, color: t.def.color, seed: this.rng.next() });
        this.emit('fire', { tower: t.id, type: 'tesla', x: t.x, y: t.y, chains: hits.length });
      }
    }
  }
  ...
  stepProjectiles(dt) {
    for (const pr of this.projectiles) {
      ...
      if (d < 18 || pr.t > 3) {
        pr.dead = true;
        this.emit('impact', { x: pr.x, y: pr.y, color: pr.color, splash: pr.splash });
        // splash damage
        const tower = this.towers.find(t => t.id === pr.from);
        const s2 = pr.splash * pr.splash;
        for (const e of this.enemies) {
          const ex = e.x - pr.x, ey = e.y - pr.y;
          if (ex * ex + ey * ey <= s2) this.damage(e, pr.damage, tower);
        }

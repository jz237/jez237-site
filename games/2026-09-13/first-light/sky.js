...
const DAY={z:[.15,.34,.74],h:[.52,.68,.86]},GOLD={z:[.10,.075,.21],h:[.78,.42,.24]},DUSK={z:[.06,.04,.15],h:[.58,.26,.28]},NIGHT={z:[.008,.012,.032],h:[.030,.040,.070]};
...
export function makeSky(scene){
 ...
 const mat=new T.ShaderMaterial({side:T.BackSide,depthWrite:false,uniforms:u,vertexShader:`varying vec3 dir;void main(){dir=position;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);}`,fragmentShader:`uniform float time,wind,windDir,night,cloud,sunElevation,skyDebug;uniform vec3 hazeColor,skyHorizon,skyZenith,sun,sunColor;varying vec3 dir;
...
void main(){vec3 d=normalize(dir);float y=max(d.y,0.);float s=max(0.,dot(d,sun));
 ...
 vec3 horizonHere=mix(coolHorizon,skyHorizon,pow(az,mix(1.6,5.,lowSun)));

...
// flank is the photo itself mapped so each body row lands on the photographed row, the back is the
// dorsal photo blended in by ring angle (v0.27.0), and the dorsal, anal, pelvic and caudal fins are
// the photo's fin pixels on a sagittal card. Both bend with the
...
export function loadFishAssets(id='largemouth'){
 ...
 cache[id]=(async()=>{
  ...
  const [flank,fins,top]=await Promise.all([loader.loadAsync(base+'flank.webp'),loader.loadAsync(base+'fins.webp'),profile.hasTop?loader.loadAsync(base+'top.webp').catch(e=>{console.warn('dorsal texture unavailable',id,e);return null;}):Promise.resolve(null)]);
  for(const t of [flank,fins,top]){if(!t)continue;t.colorSpace=T.SRGBColorSpace;t.anisotropy=8;t.wrapS=t.wrapT=T.ClampToEdgeWrapping;}
  return {profile,flank,fins,top};
 })().catch(e=>{console.warn('fish assets unavailable, using the procedural body',e);return null;});
 return cache[id];
}
function sampleProfile(st,s){const n=st.length;const f=Math.max(0,Math.min(n-1,s*(n-1)));const i=Math.floor(f),j=Math.min(n-1,i+1),t=f-i;const a=st[i],b=st[j];const L=k=>a[k]+(b[k]-a[k])*t;return {top:L('top'),bottom:L('bottom'),halfWidth:L('halfWidth'),texTop:L('texTop'),texBottom:L('texBottom'),topMid:L('topMid')||.5,topHalf:L('topHalf')||0};}
export function bodyFromProfile(profile,stations=40){
 const st=profile.stations,pos=[],uv=[],tuv=[],sta=[],belly=[],idx=[];
 ...
 for(let i=0;i<=stations;i++){const s=endS+(1-endS)*(i/stations);const p=sampleProfile(st,s);const mid=(p.top+p.bottom)/2,up=p.top-mid,down=mid-p.bottom;const hw=Math.max(.004,p.halfWidth);
  for(let j=0;j<RING;j++){const a=j/RING*Math.PI*2,c=Math.cos(a),sn=Math.sin(a);
   ...
   pos.push(x,y,s-.5);sta.push(s);belly.push(sn);tuv.push(1-s,1-(p.topMid+c*p.topHalf*.92));
 ...
 const noseC=pos.length/3;{const p=sampleProfile(st,1);pos.push(0,(p.top+p.bottom)/2,.5);sta.push(1);belly.push(0);uv.push(0,1-(p.texTop+p.texBottom)/2);tuv.push(0,1-p.topMid);const base=stations*RING;for(let j=0;j<RING;j++)idx.push(base+j,base+(j+1)%RING,noseC);}
 const tailC=pos.length/3;{const p=sampleProfile(st,endS);pos.push(0,(p.top+p.bottom)/2,endS-.5);sta.push(endS);belly.push(0);uv.push(1-endS,1-(p.texTop+p.texBottom)/2);tuv.push(1-endS,1-p.topMid);for(let j=0;j<RING;j++)idx.push((j+1)%RING,j,tailC);}
 const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(pos,3));g.setAttribute('uv',new T.Float32BufferAttribute(uv,2));g.setAttribute('station',new T.Float32BufferAttribute(sta,1));g.setAttribute('belly',new T.Float32BufferAttribute(belly,1));g.setAttribute('topUv',new T.Float32BufferAttribute(tuv,2));g.setIndex(idx);g.computeVertexNormals();return g;
}
export function finCardFromProfile(profile,segs=48,rows=10){
 const aspect=profile.texAspect,mid=profile.midRow;const pos=[],uv=[],tuv=[],sta=[],belly=[],idx=[];
 for(let i=0;i<=segs;i++){const u=i/segs,z=.5-u;for(let j=0;j<=rows;j++){const v=j/rows;pos.push(0,(mid-(1-v))*aspect,z);uv.push(u,v);sta.push(z+.5);belly.push(0);tuv.push(0,0);}}
 for(let i=0;i<segs;i++)for(let j=0;j<rows;j++){const a=i*(rows+1)+j,b=a+1,c=a+rows+1,d=c+1;idx.push(a,c,b,b,c,d);}
 const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(pos,3));g.setAttribute('uv',new T.Float32BufferAttribute(uv,2));g.setAttribute('station',new T.Float32BufferAttribute(sta,1));g.setAttribute('belly',new T.Float32BufferAttribute(belly,1));g.setAttribute('topUv',new T.Float32BufferAttribute(tuv,2));g.setIndex(idx);g.computeVertexNormals();return g;
}
export function photoMaterial(u,map,{fins=false,top=null,topTint=[1,1,1]}={}){
 const hasTop=!!top&&!fins;
 ...
 m.onBeforeCompile=s=>{Object.assign(s.uniforms,{swimPhase:u.swimPhase,swimAmp:u.swimAmp,turnBend:u.turnBend,wet:u.wet,jawOpen:u.jawOpen,jawHingeY:u.jawHingeY,topMap:{value:top},topTint:{value:new T.Vector3(topTint[0],topTint[1],topTint[2])},topOn:u.topOn||{value:1}});
  s.vertexShader=s.vertexShader.replace('#include <common>','#include <common>\nattribute float station;attribute float belly;attribute vec2 topUv;varying float vBelly;varying vec2 vTopUv;uniform float swimPhase,swimAmp,turnBend,jawOpen,jawHingeY;')
   .replace('#include <begin_vertex>',`#include <begin_vertex>
vBelly=belly;vTopUv=topUv;
...
if(jw>0.){float ang=jw*.55;float cy=transformed.y-jawHingeY,cz=transformed.z-.36;float c=cos(ang),sn=sin(ang);transformed.y=jawHingeY+cy*c-cz*sn;transformed.z=.36+cy*sn+cz*c;}`);
  s.fragmentShader=s.fragmentShader.replace('#include <common>','#include <common>\nuniform float wet;'+(hasTop?'uniform sampler2D topMap;uniform vec3 topTint;uniform float topOn;varying float vBelly;varying vec2 vTopUv;':''))
   .replace('#include <map_fragment>','#include <map_fragment>'+(hasTop?`
// the back: the dorsal photo, tinted to meet the flank at the seam, blended in over the upper flank by ring angle
vec4 tc=texture2D(topMap,vTopUv);float tb=smoothstep(.45,.9,vBelly)*tc.a*topOn;diffuseColor.rgb=mix(diffuseColor.rgb,tc.rgb*topTint,tb);`:''))
   .replace('#include <roughnessmap_fragment>','#include <roughnessmap_fragment>\nroughnessFactor=mix(roughnessFactor,.1,wet);');
 };m.customProgramCacheKey=()=>'first-light-photo-fish-v3'+(hasTop?'-top':'')+(fins?'-fins':'');return m;
}
...
export function makePhotoFishMesh(length,assets){
 ...
 const u={swimPhase:{value:0},swimAmp:{value:.02},turnBend:{value:0},wet:{value:0},jawOpen:{value:0},jawHingeY:{value:(hp.top+hp.bottom)/2},topOn:{value:1}};
 const root=new T.Group();root.scale.setScalar(length);
 const body=new T.Mesh(geo.body,photoMaterial(u,assets.flank,{top:assets.top||null,topTint:assets.profile.topTint||[1,1,1]}));body.castShadow=true;root.add(body);
 ...
 return {root,u,hasTop:!!assets.top,setTop(on){u.topOn.value=on?1:0;},setJaw(open){u.jawOpen.value=Math.max(0,Math.min(1,open||0));},setSwim(phase,amp,turn){u.swimPhase.value=phase;u.swimAmp.value=amp;u.turnBend.value=turn;pect.forEach((p,i)=>{p.g.rotation.y=-p.side*.45+Math.sin(phase*.8+i*1.5)*.12*Math.min(1,amp*25+.4);});},setWet(w){u.wet.value=w;},photo:true};
}

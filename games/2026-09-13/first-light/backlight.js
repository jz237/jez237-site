...
export const BACKLIT_GLSL=`{vec3 sv=normalize((viewMatrix*vec4(backlitSunDir,0.)).xyz);float toward=dot(normalize(-vViewPosition),sv);float k=smoothstep(.1,.85,toward)*backlitAmount;diffuseColor.rgb*=mix(vec3(1.),vec3(.36,.74,.86),k);}`;

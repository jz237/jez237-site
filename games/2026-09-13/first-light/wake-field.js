// World-space wake packets (from After the Storm) persist after the source moves on: the kayak's
// track, and later buzzbait arms and boat wakes. Height and foam share the same packets.
...
export function wakeHeight(x,z,time){let y=0;for(const w of wakeTrail){const age=time-w.time;if(w.power<=0||age<0||age>18)continue;const dx=x-w.x,dz=z-w.z;if(dx*dx+dz*dz>(4+age*.7)**2)continue;

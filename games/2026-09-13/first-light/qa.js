...
export function installQA(app){
 const api={
  ...
  demo:seed=>app.demo(seed),demoStep:(secs,dt)=>app.demoStep(secs,dt),demoReport:()=>app.demoReport(),takeRod:()=>app.takeRod(),studio:len=>app.studio(len),studioVisible:v=>app.studioVisible(v),photoFish:app.photoFish,spawnFish:(x,z,len,bold)=>app.spawnFish(x,z,len,bold),fish:()=>app.fish(),forceStrike:()=>app.forceStrike(),setHook:()=>app.setHook(),fightState:()=>app.fightState(),fightInput:i=>app.fightInput(i),releaseFish:()=>app.releaseFish(),journal:()=>app.journal()
 };

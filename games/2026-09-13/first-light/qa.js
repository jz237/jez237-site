...
export function installQA(app){
 const api={
  ...
  demo:seed=>app.demo(seed),demoStep:(secs,dt)=>app.demoStep(secs,dt),demoReport:()=>app.demoReport(),takeRod:()=>app.takeRod(),gallery:(id,t)=>app.gallery(id,t),forage:()=>app.forage(),conditions:()=>app.conditions(),setDate:(y,m,d)=>app.setDate(y,m,d),setPaddle:(p,t)=>app.setPaddle(p,t),forceSnag:()=>app.forceSnag(),voice:()=>app.voice(),session:(v,o)=>app.session(v,o),photo:on=>app.photo(on),tournament:o=>app.tournament(o),cat:()=>app.cat(),tutorial:()=>app.tutorial(),tutorialState:()=>app.tutorialState(),tutorialSkip:()=>app.tutorialSkip(),tournamentState:()=>app.tournamentState(),endTournament:()=>app.endTournament(),photoOrbit:(y,p,d)=>app.photoOrbit(y,p,d),savePhoto:k=>app.savePhoto(k),sessionState:()=>app.sessionState(),endSession:()=>app.endSession(),sessionSubmit:i=>app.sessionSubmit(i),leaveSession:()=>app.leaveSession(),say:ev=>app.say(ev),retie:()=>app.retie(),setAbrasion:v=>app.setAbrasion(v),unlocked:()=>app.unlocked(),setWeatherLive:l=>app.setWeatherLive(l),forageMesh:()=>app.forageMesh(),forageTo:(i,x,z)=>app.forageTo(i,x,z),journalAdd:c=>app.journalAdd(c),galleryState:()=>app.galleryState(),galleryLake:()=>app.galleryLake(),closeGallery:()=>app.closeGallery(),studio:(len,species)=>app.studio(len,species),studioVisible:v=>app.studioVisible(v),photoFish:app.photoFish,spawnFish:(x,z,len,bold,species)=>app.spawnFish(x,z,len,bold,species),fish:()=>app.fish(),forceStrike:()=>app.forceStrike(),setHook:()=>app.setHook(),fightState:()=>app.fightState(),fightInput:i=>app.fightInput(i),releaseFish:()=>app.releaseFish(),journal:()=>app.journal()
 };

...
export function makeWildlife(scene,{perch,cove}){
 ...
 const N=320,ipos=new Float32Array(N*3),iseed=new Float32Array(N);for(let i=0;i<N;i++){ipos[i*3]=(Math.random()-.5)*24;ipos[i*3+1]=.3+Math.random()*1.3;ipos[i*3+2]=(Math.random()-.5)*24;iseed[i]=Math.random();}
 ...
 const im=new T.ShaderMaterial({transparent:true,depthWrite:false,blending:T.AdditiveBlending,uniforms:iu,vertexShader:`attribute float seed;uniform float time,density;uniform vec3 center;varying float a;void main(){vec3 p=position;p.x+=sin(time*.7+seed*6.28)*.6;p.y+=sin(time*1.9+seed*9.1)*.25;p.z+=cos(time*.6+seed*4.4)*.6;p=center+(mod(p-center+12.,24.)-12.);vec4 mv=modelViewMatrix*vec4(p,1.);gl_Position=projectionMatrix*mv;float d=max(1.,-mv.z);gl_PointSize=clamp(18./d,1.,3.);a=.55*density*(.35+.65*abs(sin(time*3.+seed*20.)))*smoothstep(12.,3.,d)*step(seed,density);}`,fragmentShader:`varying float a;void main(){vec2 c=gl_PointCoord-.5;float r=dot(c,c);if(r>.25)discard;gl_FragColor=vec4(.95,.9,.75,a*(1.-r*4.));}`});

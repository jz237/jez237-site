...
const hud=mountHud({
 ...
 onSteady:v=>{settings.steadyCamera=v;saveSettings(settings);},onVoice:v=>{settings.voice=v;saveSettings(settings);voice.setVolume(v);},onLiveWeather:v=>{settings.liveWeather=v;saveSettings(settings);liveWeatherTick(true);},onFov:v=>{settings.fov=v;saveSettings(settings);camera.fov=v;camera.updateProjectionMatrix();},
...
const fishAssetList=await Promise.all(ROSTER.map(id=>loadFishAssets(id)));const fishAssets=fishAssetList[0];const assetsById=Object.fromEntries(ROSTER.map((id,i)=>[id,fishAssetList[i]]));const pop=makePopulation(scene,bathy,cover.features,{assets:assetsById,roster:ROSTER});let studio=null;

...
*
* version 1.3.11 (check src/README.txt for additional information)
*
...
/**
* Fake WebAudio buffer used as startup optimization.
*/
class SurrogateAudioBuffer {
	constructor(numberOfChannels, len) {
		this.numberOfChannels = numberOfChannels;
		this.channelData = [new Float32Array(len), new Float32Array(len)];
	}
	getChannelData(ch) {
...
class AudioBackendAdapterBase {
	...
	/**
	* Optimization to avoid WebAudio playback stutter when starting new song.
	*/
	cacheStartupAudioBuffer() { return false; }
...
//////////// playback related ////////////
		...
		{
			if (this._fakeProcessingEvent != null)
			{
				// buffer was previously rendered.. just copy the data
				let	dest = e.outputBuffer;
				let src = this._fakeProcessingEvent.outputBuffer;
								
				for (let ch = 0; ch < dest.numberOfChannels; ch++)
				{
					dest.getChannelData(ch).set(src.getChannelData(ch));
				}
				this._fakeProcessingEvent  = null;
				return;
			}
		...
		*/
		_initIfNeeded: function (virtualName, data, options)
		{
			...
			else if (status === 0)
			{
				...
				// render 1st audio buffer in advance to avoid startup stutter
				let p = ScriptNodePlayer.getInstance();
				if(p._backendAdapter.cacheStartupAudioBuffer())
				{
					p._fakeProcessingEvent = null;
					
					let buf = new SurrogateAudioBuffer(p._backendAdapter.getChannels(), p._backendAdapter.getProcessorBufSize());					
					let fakeEvent = { outputBuffer: buf };
					p._genSamples(fakeEvent);
					
					p._fakeProcessingEvent = fakeEvent;
				}
				...
				xhr.onload = function (oEvent) {
					...
					else
					{
						...
						if (arrayBuffer)
						{
							...
							let f = this._backendAdapter.registerFileData(pfn, fileData);
...
//////////// callbacks triggered by backend logic ////////////
		...
		*/
		...
		{
			if ((typeof this._backendAdapter.Module == 'undefined') || (typeof this._backendAdapter.Module.UTF8ToString == 'undefined'))
			{
				console.log("error: _backendAdapter.Module.UTF8ToString() MUST be defined in order to use _fileRequestCallback()");
			}
			...
			let r= this._preloadFile(virtualName,
									function() {
										...
										let success = this._initIfNeeded(this.mainVirtualName, this.mainData, this.mainOptions);
		...
		*/
		_fileDataRequestCallback: function(namePtr)
		{
			let virtualNameBackend = this._backendAdapter.Module.UTF8ToString(namePtr);
...
//      original old callback based APIs)
		...
		*/
		playSong(backend, url, options, doOnTrackEnd, spectrumEnabled, externalTicker, onProgress)
		{
			return new Promise((resolve, reject) => {
				ScriptNodePlayer.initialize(backend, doOnTrackEnd, [], spectrumEnabled, externalTicker)	// no use of 'requiredFiles'.. rarely used anyway 
				.then((msg) => { // resolved		
									// map below 2 callbacks to the result of the Promise 
									let onTrackReadyToPlay = function(filename) {	
										ScriptNodePlayer.getInstance().play();						// hard coded autoplay			
										resolve({ "status": "success", "ctx": "loadMusicFromURL", "file": url });
									}.bind(this);

									let onFail = function(filename) {							
										reject({ "status": "fail", "ctx": "loadMusicFromURL", "file": url });						
									}.bind(this);

									ScriptNodePlayer.getInstance().loadMusicFromURL(url, options, undefined, onFail, onProgress, onTrackReadyToPlay);	// no use of onCompletion.. rather useless anyway
								},
						(failMsg) => { // rejected
									reject({ "status": "fail", "ctx": "ScriptNodePlayer.initialize", "file": url });						
								}
				);
			});
		}

...
const reset=()=>{const d=Math.max(22.6,24.5/Math.max(camera.aspect,.32));controls.target.set(0,.65,0);camera.position.set(.12,.65,d);controls.update()};reset();
...
for(const [x,y,z,w,h,power] of [[0,8,7,18,2,7],[-9,3,5,2,12,4],[10,1,3,1,10,5],[0,-7,8,15,.45,2],[1,2,-9,7,3,3]]){const panel=new T.Mesh(new T.PlaneGeometry(w,h),new T.MeshBasicMaterial({color:new T.Color(power,power*1.05,power*1.15),side:T.DoubleSide}));panel.position.set(x,y,z);panel.lookAt(0,0,0);env.add(panel)}
...
window.skynet={scene,camera,controls,renderer,composer,reset,setTime(t){elapsed=t;update(0)},stats(){let meshes=0,extrusions=0;root.traverse(o=>{if(o.isMesh)meshes++;if(o.geometry?.type==='ExtrudeGeometry')extrusions++});return{version:'2026.09.05.2',beaconCount,beaconTime:beacons[0].uniforms.time.value,meshes,extrusions,frames,elapsed,camera:camera.position.toArray(),distance:camera.position.distanceTo(controls.target),glow:bloom.strength,autoOrbit:controls.autoRotate,animated,drawCalls:renderer.info.render.calls}},ready:true};$('loading').remove();

import {authoredMotion} from './authored-motion.mjs?v=17';
...
async function buildFighter(id,index){
 ...
 const authored=authoredMotion(model);const mixer=new THREE.AnimationMixer(model),actions={},durations={};
 ...
 for(const [name,clip] of Object.entries(authored)){actions[name]=mixer.clipAction(clip);actions[name].setLoop(name==='guard'?THREE.LoopRepeat:THREE.LoopOnce);actions[name].clampWhenFinished=true;durations[name]=clip.duration;}
 actions.idle=actions.guard;durations.idle=1;
 ...
 const profiles=Object.fromEntries(Object.entries(MOVES).map(([name,m])=>[name,{...m}]));
...
function play(v,name,duration){
 ...
 a.setLoop(['idle','guard','walk_fwd','walk_back'].includes(name)?THREE.LoopRepeat:THREE.LoopOnce);a.reset().setEffectiveWeight(1).setEffectiveTimeScale(rate);a.enabled=true;a.play();v.current=a;
}
let battle=new Combat(237),accumulator=0,lastTime=0,paused=false,speed=1,eventLife=0,sound=false,audio=null,study=null;
...
function ui(){for(let i=0;i<2;i++){$(`#hp${i}`).style.width=`${battle.fighters[i].hp}%`;$(`#wins${i}`).textContent=Array.from({length:2},(_,n)=>n<battle.wins[i]?'●':'○').join(' ');}$('#timer').textContent=String(Math.ceil(battle.time)).padStart(2,'0');$('#round').textContent=`ROUND ${battle.round}`;
 $('#banner').innerHTML=study?'':battle.phase==='intro'?(battle.phaseTime>1?`ROUND ${battle.round}<small>CPU EXHIBITION</small>`:'FIGHT'):battle.phase==='outro'?(battle.winner<0?'DRAW':`${battle.winner===0?'JEZ':'BENNY'} WINS`)+`<small>${battle.wins.some(n=>n===2)?'MATCH COMPLETE · NEW MATCH SHORTLY':'NEXT ROUND SHORTLY'}</small>`:'';
}
...
$('#restart').onclick=()=>{study=null;$('#studyMove').value='';document.querySelector('.hud').hidden=false;battle=new Combat((battle.seed+1)>>>0,visuals.map(v=>v.profiles));accumulator=0;effects.clear();visuals.forEach(v=>{v.serial=-1;v.state=null;});};
...
$('#studyMove').onchange=e=>{study=e.target.value||null;if(!study){battle=new Combat(237,visuals.map(v=>v.profiles));accumulator=0;}effects.clear();document.querySelector('.hud').hidden=Boolean(study);paused=false;$('#pause').textContent='Pause fight';for(const v of visuals){v.current=null;v.displayPose=null;v.serial=-1;if(study){play(v,study,0);v.current.setLoop(THREE.LoopRepeat);}}};
$('#testEffects').onclick=()=>{effects.clear();for(let i=0;i<2;i++)effects.impact({type:i?'block':'hit',target:i,move:'cross'},battle.fighters[i],MOVES.cross);};
...
try{await Promise.all([buildFighter('jez',0),buildFighter('benny',1)]);battle.profiles=visuals.map(v=>v.profiles);$('#loading').hidden=true;const requestedStudy=new URLSearchParams(location.search).get('study');if(['guard','jab','cross','hook','lowkick'].includes(requestedStudy)){$('#studyMove').value=requestedStudy;$('#studyMove').dispatchEvent(new Event('change'));}
 let frames=0,frameTime=0,slowWindows=0;renderer.setAnimationLoop(t=>{if(document.hidden)return;let realDt=lastTime?Math.min((t-lastTime)/1000,.05):0;lastTime=t;let dt=paused?0:realDt*speed;accumulator+=dt;while(accumulator>=1/60){if(!study)battle.step(1/60);accumulator-=1/60;}

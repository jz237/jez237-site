...
export class Combat {
 ...
 resetRound(){this.round++;this.time=60;this.phase='intro';this.phaseTime=2.4;this.fighters=[0,1].map((id)=>({id,x:id?1.42:-1.42,previousX:id?1.42:-1.42,face:id?-1:1,hp:100,state:'idle',t:0,duration:0,move:null,hit:false,think:.3+id*.3,velocity:0,serial:0}));}
 ...
 step(dt){
  ...
  for(const f of this.fighters){const o=this.fighters[1-f.id];f.t+=dt;f.think-=dt;
   ...
   if(['idle','walk','back'].includes(f.state)&&f.think<=0){const distance=Math.abs(o.x-f.x);f.think=.12+this.random()*.20;
    if(o.state==='attack'&&o.t<this.move(o).hit&&distance<1.65&&this.random()<.53)this.act(f,this.random()<.22?'dodge':'block',.4);
    else if(distance>1.16){if(f.state!=='walk')this.act(f,'walk',0);}
    else if(this.random()<.67){const choices=distance>1.08?['kick','high']:['jab','jab','hook','uppercut','kick'];const move=choices[Math.floor(this.random()*choices.length)];this.act(f,'attack',MOVES[move].duration,move);f.think=MOVES[move].duration+.08;}
    else if(this.random()<.45){if(f.state!=='back')this.act(f,'back',0);f.think=.35;}
    else this.act(f,'idle',0);
   }
   const target=f.state==='walk'?f.face*1.25:f.state==='back'?-f.face*.9:0;f.velocity+=(target-f.velocity)*(1-Math.exp(-dt*15));f.x=Math.max(-1.43,Math.min(1.43,f.x+f.velocity*dt));
  }
  const [a,b]=this.fighters;if(b.x-a.x<.84){const mid=Math.max(-1.01,Math.min(1.01,(a.x+b.x)/2));a.x=mid-.42;b.x=mid+.42;}

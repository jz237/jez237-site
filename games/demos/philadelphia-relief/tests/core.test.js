...
test('presets', async (t) => {
  ...
  await t.test('the tour opens on the skyline and visits every shot once', () => {
    ...
    assert.deepEqual([...new Set(TOUR.map((s) => s.preset))].sort(), PRESET_IDS.filter((id) => id !== 'architecture').sort());
    assert.equal(TOUR.length, PRESETS.length - 1);
  });
...
test('url state', async (t) => {
  ...
  await t.test('out-of-range values in a hash are clamped, not honoured', () => {
    ...
    assert.equal(CAMERA.camDist.min, 900, 'source-quality camera floor');
  });

/* Corner Pocket II — audio: WebAudio synth SFX (no assets) + speech callouts.
...
const AU = (() => {
  ...
  let muted = localStorage.getItem('cp2-mute') === '1';
  ...
  function setMute(m){
    muted = m; localStorage.setItem('cp2-mute', m?'1':'0');
  ...
  /* ---------- primitives ---------- */
  function osc(type,freq,t0,dur,vol,slideTo){
    if (!ctx) return;
  ...
  const SFX = {
    ...
    /* THE pool-ball click — pitched by impact speed */
    click(sp){ const t=now(); const v = Math.min(0.55, 0.16 + sp/2200);
      noise(t,.018,v,2400,5200); noise(t,.03,v*0.5,1200,2600);
      osc('sine', 950 + sp*0.3, t, .03, v*0.35, 500); },
    cash(){ const t=now();       // the register: bell + drawer
      osc('sine',1568,t,.5,.22); osc('sine',2093,t+.02,.4,.14);
      noise(t+.1,.09,.3,300,900); noise(t+.16,.06,.2,500,1600); },
    ...
    spinTick(){ const t=now(); osc('square',1560,t,.025,.09); noise(t,.015,.12,2000,4800); },
    hole(){ const t=now(); noise(t,.08,.35,300,900); osc('sine',120,t,.16,.4,60); },
    lock(){ const t=now(); noise(t,.06,.5,200,700); osc('sine',88,t,.3,.6,48);
      osc('square',262,t+.12,.12,.12); osc('square',330,t+.24,.14,.12); },
    ...
    scratch(){ const t=now(); osc('sawtooth',220,t,.4,.2,90); osc('square',110,t+.1,.3,.2,60);
      noise(t,.15,.2,200,700); },
    magnetGrab(){ const t=now(); osc('sawtooth',60,t,.9,.3,52);
      for (let i=0;i<7;i++) noise(t+i*.11,.05,.16,1800,5200); },
    magnetFling(){ const t=now(); noise(t,.08,.5,600,2800,4000); osc('sine',180,t,.14,.4,760); },
    kickback(){ const t=now(); noise(t,.06,.6,200,900); osc('sine',95,t,.16,.6,300); },
    jackpot(){ const t=now();
      [659,784,1047,1319,1568].forEach((f,i)=>{ osc('square',f,t+i*.07,.2,.16); osc('sine',f*2,t+i*.07,.24,.08); });
      noise(t+.35,.1,.3,400,1200); },
    multiball(){ const t=now();
      [262,330,392,523,659,784,1047].forEach((f,i)=>{ osc('square',f,t+i*.08,.24,.15); });
      osc('sine',65,t,1.2,.4,50); },
    sidebet(){ const t=now(); [880,740,988,830,1109].forEach((f,i)=>osc('triangle',f,t+i*.06,.12,.13)); },
    runTable(){ const t=now();
      [523,659,784,1047].forEach((f,i)=>{ osc('square',f,t+i*.09,.2,.15); });
      SFX.cash(); },
    ...
    jingle(){ const t=now();
      [330,415,494,659].forEach((f,i)=>{ osc('square',f,t+i*.11,.18,.14); }); },
    over(){ const t=now();
  ...
  function say(text, prio){
    ...
    try{
      ...
      u.rate = 0.88; u.pitch = 0.28; u.volume = 1;
      u.onend = () => {
  ...
  return { init, resume, setMute, toggleMute, get muted(){return muted;},
           sfx: SFX, say, shutUp, get ready(){return ready;} };
})();

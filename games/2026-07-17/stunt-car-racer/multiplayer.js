...
var SCR_Multiplayer = (function () {
  ...
  function createPeerConnection() {
    ...
    pc.onicecandidate = function (e) {
      if (e.candidate) {
        sigSend({ type: 'ice', candidate: e.candidate }).catch(function (err) {
  ...
  function addOrQueueIceCandidate(candidate) {
  ...
  function maybeReportOpen() {
    if (!connected &&
        dcGame && dcGame.readyState === 'open' &&
        dcReliable && dcReliable.readyState === 'open') {
      connected = true;
      if (onOpen) onOpen();
    }
  }
      ...
      return new Promise(function (resolve) {
        startPolling(function (msg) {
          if (msg.type === 'answer') {
            ...
            addOrQueueIceCandidate(msg.candidate)
            ...
            pc.createOffer().then(function (offer) {
              ...
              return sigSend({ type: 'offer', sdp: pc.localDescription });
            }).catch(function (e) { console.error('offer flow:', e); });
          }
        }, function () {
  ...
  function join(url, code) {
    ...
    return sigPost('/rooms/' + roomCode + '/join').then(function (data) {
      ...
      // Notify host we've joined, then start polling for its offer and ICE.
      return sigSend({ type: 'joined' }).then(function () {
        ...
        startPolling(function (msg) {
          if (msg.type === 'offer') {
            pc.setRemoteDescription(new RTCSessionDescription(msg.sdp))
              ...
              .then(function () {
                return sigSend({ type: 'answer', sdp: pc.localDescription });
              })
